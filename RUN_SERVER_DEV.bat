...
powershell -NoProfile -ExecutionPolicy Bypass -Command "& { $dd='%DD_EXE%'; $dmb='%~dp0tgstation.dmb'; $log='%~dp0data\server_output.log'; $pidFile='%~dp0PID.txt'; while ($true) { if (Test-Path $log) { Remove-Item $log -Force }; Write-Host ('[{0}] Starting server...' -f (Get-Date)); $psi = New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName = $dd; $psi.Arguments = '"""' + $dmb + '""" 1337 -trusted -close -verbose'; $psi.UseShellExecute = $false; $psi.RedirectStandardOutput = $true; $psi.RedirectStandardError = $true; $proc = [System.Diagnostics.Process]::Start($psi); $proc.Id | Out-File -FilePath $pidFile -Encoding UTF8; Write-Host ('PID: {0}' -f $proc.Id); $out = $proc.StandardOutput.ReadToEnd(); $err = $proc.StandardError.ReadToEnd(); $proc.WaitForExit(); if (Test-Path $pidFile) { Remove-Item $pidFile -Force }; ($out + $err) | Out-File -FilePath $log -Encoding UTF8; Write-Host ('[{0}] Server stopped.' -f (Get-Date)); if (Test-Path $log) { Write-Host '=== Last run output ==='; Get-Content $log | Write-Host; Write-Host '=== End of log ===' }; Write-Host 'Restarting in 5 seconds...'; Start-Sleep -Seconds 5 } }"
