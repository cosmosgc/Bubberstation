...
/atom/movable/screen/vomit_jump/examine(mob/user)
	...
	. += span_notice("Rastreia todos os vômitos da nebulosa gerados pelas pessoas que você tocou.")
	. += span_notice("Clique com o botão esquerdo enquanto mergulha no espaço para se mover para um vômito de nebulosa, se houver algum.")
	. += span_notice("A nebulosa atual vomita:[vomits.len]")
...
/atom/movable/screen/vomit_jump/Click(location, control, params)
	...
	if(!istype(usr.loc, /obj/effect/dummy/phased_mob/space_dive))
		usr.balloon_alert(usr, "Deve ser mergulho espacial!")
		return

	var/list/vomits = get_valid_vomits()

	if(!vomits.len)
		usr.balloon_alert(usr, "Nenhuma nebulosa vomita!")
		return
