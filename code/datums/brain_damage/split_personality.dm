...
/datum/brain_trauma/severe/split_personality/brainwashing/get_ghost()
	set waitfor = FALSE
	var/mob/chosen_one = SSpolling.poll_ghosts_for_target("Do you want to play as [span_danger("[owner.real_name]'s")] brainwashed mind?", poll_time = 7.5 SECONDS, checked_target = stranger_backseat, alert_pic = owner, role_name_text = "Lavagem cerebral")
	if(chosen_one)
...
/datum/brain_trauma/severe/split_personality/blackout/on_life(seconds_per_tick)
	...
	if(prob(10) && !HAS_TRAIT(owner, TRAIT_DISCOORDINATED_TOOL_USER))
		...
		owner.balloon_alert(owner, "Destreza reduzida temporária!")
