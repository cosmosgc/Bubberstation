...
/datum/terror_handler/phobia_source/proc/handle_speech(datum/source, list/speech_args)
	...
	to_chat(owner, span_warning("Você luta para dizer a palavra\"[span_phobia("[trigger_regex.group[2]]")]\"!"))
...
/datum/terror_handler/phobia_source/proc/freak_out(reason)
	...
	if(isatom(reason))
		...
		to_chat(owner, span_bolddanger("Vende[span_phobia("[as_atom.name]")] [message]!"))
	else
		to_chat(owner, span_bolddanger("Algo.[message]!"))
