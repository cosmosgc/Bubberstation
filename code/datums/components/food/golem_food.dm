...
/datum/component/golem_food/proc/on_attack(atom/source, mob/living/target, mob/living/user, list/modifiers)
	...
	if (extra_validation && !extra_validation.Invoke())
		source.balloon_alert(user, "Não comestível!")
		return COMPONENT_CANCEL_ATTACK_CHAIN
	if (!snack_type.can_consume(target))
		source.balloon_alert(user, "Não posso consumir!")
		return COMPONENT_CANCEL_ATTACK_CHAIN
...
/obj/item/food/golem_food
	...
	desc = "Você não deveria ser capaz de ver isso. Este é um item abstrato que existe para permitir que você coma pedras."
	bite_consumption = 2
