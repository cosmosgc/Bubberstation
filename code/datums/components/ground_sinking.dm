...
/datum/component/ground_sinking/proc/sinking_progress(mob/living/basic/living_target)
	living_target.visible_message(span_notice("[living_target]Começa a afundar no chão!"))
	for(var/i in 1 to 3)
...
/datum/component/ground_sinking/proc/start_regenerating()
	...
	if (living_parent.health == living_parent.maxHealth)
		return
	living_parent.visible_message(span_notice("[living_parent]As feridas começam a tricotar!"))
