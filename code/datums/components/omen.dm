...
/datum/component/omen/proc/check_accident(atom/movable/our_guy)
	...
	if(prob(0.001) && (living_guy.stat != DEAD)) // You hit the lottery! Kinda.
		living_guy.visible_message(span_danger("[living_guy] suddenly bursts into flames!"), span_danger("De repente você pegou fogo!"))
	...
	for(var/obj/machinery/door/airlock/darth_airlock in our_guy_pos)
		...
		to_chat(living_guy, span_warning("Uma força malévola lança seu corpo para o chão..."))
	...
	for(var/turf/the_turf as anything in get_adjacent_open_turfs(living_guy))
		...
		if(living_guy.can_z_move(DOWN, the_turf, z_move_flags = ZMOVE_FALL_FLAGS))
			to_chat(living_guy, span_warning("Uma força malévola te guia até a borda..."))
		...
		for(var/obj/structure/mirror/evil_mirror in the_turf)
			to_chat(living_guy, span_warning("Você passa pelo espelho e olha para ele..."))
			if(evil_mirror.broken)
				to_chat(living_guy, span_notice("Você se sente com sorte, de alguma forma."))
				return
			switch(rand(1, 5))
				if(1)
					to_chat(living_guy, span_warning("O espelho explode em milhões de pedaços! Espere, isso significa que você é ainda mais azarado?"))
					evil_mirror.take_damage(evil_mirror.max_integrity, BRUTE, MELEE, FALSE)
					if(prob(50 * effective_luck)) // sometimes
						luck_mod += 0.25
						damage_mod += 0.25
				if(2 to 3)
					to_chat(living_guy, span_big(span_hypnophrase("Oh Deus, você não pode ver seu reflexo!")))
					if(HAS_TRAIT(living_guy, TRAIT_NO_MIRROR_REFLECTION)) // not so living i suppose
						to_chat(living_guy, span_green("Bem, obviamente."))
						return
					INVOKE_ASYNC(living_guy, TYPE_PROC_REF(/mob, emote), "scream")

				if(4 to 5)
					if(HAS_TRAIT(living_guy, TRAIT_NO_MIRROR_REFLECTION))
						to_chat(living_guy, span_warning("Você não vê nada de novo. Huh."))
						return
					to_chat(living_guy, span_userdanger("Você vê seu reflexo, mas ele está sorrindo malevolentemente e olhando diretamente para você!"))
					INVOKE_ASYNC(living_guy, TYPE_PROC_REF(/mob, emote), "scream")

			living_guy.set_jitter_if_lower(25 SECONDS)
			if(prob(7 * effective_luck))
				to_chat(living_guy, span_warning("Você está completamente chocado com esta mudança de eventos!"))
				to_chat(living_guy, span_userdanger("Você agarra seu coração!"))
...
/datum/component/omen/proc/check_slip(mob/living/our_guy, amount)
	...
	if(prob(30)) // AAAA
		...
		to_chat(our_guy, span_warning("Que noite horrível... Ter uma maldição!"))

	if(prob(30 * luck_mod) && our_guy.get_bodypart(BODY_ZONE_HEAD)) /// Bonk!
		playsound(our_guy, 'sound/effects/tableheadsmash.ogg', 90, TRUE)
		our_guy.visible_message(span_danger("[our_guy] hits [our_guy.p_their()] head really badly falling down!"), span_userdanger("Você bateu a cabeça muito mal caindo!"))
...
/datum/component/omen/proc/check_bless(mob/living/our_guy, mob/living/priest, obj/item/book/bible/bible, bless_result)
	...
	to_chat(our_guy, span_green("Você se sente fantástico!"))
	qdel(src)
