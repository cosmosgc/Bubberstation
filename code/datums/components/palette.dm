...
/datum/component/palette/proc/on_examine(datum/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("<b>Click direito</b>Este item enquanto está em sua mão ativa para abrir/fechar seu menu de seleção de cores.")
	examine_list += span_notice("No coletor de cores,<b>Clique à esquerda.</b>um botão de cor para pegá-lo ou<b>Click direito</b>para removê-lo.")
