	...
	if(physical_cash_total < total_cost)
		...
		if(armless)
			if(!user.pulling || !iscash(user.pulling) && !istype(user.pulling, /obj/item/card/id))
				to_chat(user, span_notice("Tente pegar uma identidade válida, dinheiro do espaço, holochip ou moeda enquanto usa\the [parent]!"))
				return FALSE
		return FALSE

	if(physical_cash_total < total_cost)
		to_chat(user, span_warning("Fundos é incapaz. Abortando."))
		return FALSE
