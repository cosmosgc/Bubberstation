...
/datum/symptom/disfiguration/Activate(datum/disease/advance/disease)
	...
	switch(disease.stage)
		if(5)
			...
			victim.visible_message(span_warning("[victim]'s face appears to cave in!"), span_notice("Você sente seu rosto amassado e cair!"))
		else
			victim.visible_message(span_warning("[victim]'s face begins to contort..."), span_notice("Seu rosto está molhado e maleável..."))
