	...
	switch(A.stage)
		...
		if(3)
			...
			if(living_mob.on_fire) //check to make sure they actually caught on fire, or if it was prevented cause they were wet.
				living_mob.visible_message(span_warning("[living_mob] Pega nevoeiro!"), ignored_mobs = living_mob)
...
/datum/symptom/alkali/Activate(datum/disease/advance/A)
	...
	switch(A.stage)
		...
		if(4)
			if(M.fire_stacks < 0)
				M.visible_message(span_warning("[M] Está suando e estourando em contato com água!"))
