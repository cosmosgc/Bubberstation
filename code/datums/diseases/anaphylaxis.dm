	...
	switch(stage)
		...
		if(1)
			if(affected_mob.num_hands >= 1 && SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_warning("Você sente sua mão[affected_mob.num_hands == 1 ? "":"s"]Comece a tremer."))
			...
			if(affected_mob.num_legs >= 1 && SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_warning("Você sente sua perna[affected_mob.num_hands == 1 ? "":"s"]Comece a tremer."))
				affected_mob.adjust_jitter_up_to(4 SECONDS * seconds_per_tick, 1 MINUTES)
		...
		if(2)
			...
			if(affected_mob.num_hands >= 1 && SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_warning("Você sente sua mão[affected_mob.num_hands == 1 ? "":"s"]Agite violentamente."))
				affected_mob.adjust_jitter_up_to(8 SECONDS * seconds_per_tick, 1 MINUTES)
				if(prob(20))
					affected_mob.drop_all_held_items()
			if(affected_mob.num_legs >= 1 && SPT_PROB(5, seconds_per_tick))
				to_chat(affected_mob, span_warning("Você sente sua perna[affected_mob.num_hands == 1 ? "":"s"]Agite violentamente."))
				affected_mob.adjust_jitter_up_to(8 SECONDS * seconds_per_tick, 1 MINUTES)
