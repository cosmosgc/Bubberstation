...
/datum/disease/anxiety/stage_act(seconds_per_tick)
	...
	switch(stage)
		...
		if(4)
			...
			if(SPT_PROB(2.5, seconds_per_tick))
				affected_mob.visible_message(span_danger("[affected_mob] tropeça em pânico."), 												span_userdanger("Você tem um ataque de pânico!"))
				affected_mob.adjust_confusion(rand(6 SECONDS, 8 SECONDS))
				affected_mob.adjust_jitter(rand(12 SECONDS, 16 SECONDS))
			if(SPT_PROB(1, seconds_per_tick))
				affected_mob.visible_message(span_danger("[affected_mob] Tossir borboletas!"), 													span_userdanger("Você tosse borboletas!"))
