/datum/disease/chronic_illness
	...
	desc = "Uma doença descoberta em um laboratório Interdyne causada pela sujeição à tecnologia de correção de fluxo temporal.\
É completamente incurável, embora possa ser tratado para reverter sua progressão e não contágio.\
Se não for tratada, o sujeito sofrerá de vários sintomas,\
incluindo, mas não limitado a tontura, náusea, palpitações cardíacas, e nos estágios finais, morte."
		...
		if(5)
			switch(rand(1,2))
				...
				if(2)
					...
					affected_mob.visible_message(span_warning("[affected_mob] is erased from the timeline!"), span_userdanger("Você está arrancado da linha do tempo!"))
