/datum/disease/death_sandwich_poisoning
	name = "Death Sandwich Poisoning"
	desc = "Se não for tratado, o sujeito acabará por perecer."
	form = "Condition"
	max_stages = 3
	cure_text = /datum/reagent/toxin/anacea::name // I ain'Estou prestes a fazer um segundo sanduíche para neutralizar o primeiro.'m going for is this.
	cures = list(/datum/reagent/toxin/anacea)
	cure_chance = 4
	agent = "Eating the Death Sandwich wrong"
	spread_text = "None"
	viable_mobtypes = list(/mob/living/carbon/human)
	severity = DISEASE_SEVERITY_DANGEROUS
	disease_flags = CURABLE
	spread_flags = DISEASE_SPREAD_SPECIAL
	visibility_flags = HIDDEN_SCANNER
	bypasses_immunity = TRUE
	required_organ = ORGAN_SLOT_STOMACH

/datum/disease/death_sandwich_poisoning/stage_act(seconds_per_tick)
	. = ..()
	if(!.)
		return

	switch(stage)
		if(1)
			if(SPT_PROB(1.5, seconds_per_tick))
				affected_mob.emote("cough")
			if(SPT_PROB(0.5, seconds_per_tick))
				affected_mob.emote("gag")
			if(SPT_PROB(0.5, seconds_per_tick))
				affected_mob.adjust_tox_loss(5)
		if(2)
			if(SPT_PROB(5, seconds_per_tick))
				affected_mob.emote("cough")
			if(SPT_PROB(2.5, seconds_per_tick))
				affected_mob.emote("gag")
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger("Seu corpo está quente!"))
				if(prob(20))
					affected_mob.take_bodypart_damage(burn = 1)
			if(SPT_PROB(3, seconds_per_tick))
				affected_mob.adjust_tox_loss(10)

		if(3)
			if(SPT_PROB(5, seconds_per_tick))
				affected_mob.emote("gag")
			if(SPT_PROB(10, seconds_per_tick))
				affected_mob.emote("gasp")
			if(SPT_PROB(2.5, seconds_per_tick))
				affected_mob.vomit(VOMIT_CATEGORY_BLOOD, lost_nutrition = 20)
			if(SPT_PROB(2.5, seconds_per_tick))
				to_chat(affected_mob, span_danger("Seu corpo está quente!"))
				if(prob(60))
					affected_mob.take_bodypart_damage(burn = 2)
			if(SPT_PROB(6, seconds_per_tick))
				affected_mob.adjust_tox_loss(15)
			if(SPT_PROB(1.5, seconds_per_tick))
				to_chat(affected_mob, span_danger("Você tenta gritar, mas nada sai!"))
				affected_mob.set_silence_if_lower(5 SECONDS)
