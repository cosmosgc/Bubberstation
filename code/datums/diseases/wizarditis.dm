/datum/disease/wizarditis
	...
	desc = "Alguns especulam que este vírus é a causa da existência da Federação do Feiticeiro Espacial. Os indivíduos afetados mostram sinais de dano cerebral, gritando frases obscuras ou sem sentido. Nos estágios finais, os sujeitos expressam, em algum momento, os sentimentos de poder interior, e citam \"a capacidade de controlar as forças do próprio cosmos!\" Um gole de espíritos fortes e viris geralmente os reverte à condição normal, humana. Uma forma mágica de aterramento pode ajudar, mas não a curará sozinha."
	...
	switch(stage)
		if(2)
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger("Você sente[pick("that you don't have enough mana", "that the winds of magic are gone", "an urge to summon familiar")]."))

		if(3)
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger("Você sente[pick("the magic bubbling in your veins", "that this location gives you a +1 to INT", "an urge to summon familiar")]."))
				spawn_wizard_clothes(10)

		if(4)
			if(SPT_PROB(1, seconds_per_tick))
				to_chat(affected_mob, span_danger("Você sente[pick("the tidal wave of raw power building inside", "that this location gives you a +2 to INT and +1 to WIS", "an urge to teleport")]."))
