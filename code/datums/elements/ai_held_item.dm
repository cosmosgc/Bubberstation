...
/datum/element/ai_held_item/proc/on_click(mob/living/source, mob/living/user)
	...
	source.visible_message(span_danger("[source] Gotas [carried_item] em [user] Pés!"))
...
/datum/element/ai_held_item/proc/on_examined(mob/living/source, mob/user, list/examine_text)
	...
	if (!carried_item)
		return
	examine_text += span_notice("[source.p_They()] [source.p_are()] carregando [carried_item.examine_title(user)].")
...
/datum/element/ai_held_item/proc/on_death(mob/living/ol_yeller)
	...
	ol_yeller.visible_message(span_danger("[ol_yeller] Gotas [carried_item] Como [ol_yeller.p_they()] Morra [ol_yeller.p_s()]."))
