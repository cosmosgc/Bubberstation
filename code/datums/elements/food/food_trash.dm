...
/datum/element/food_trash/proc/food_crossed(datum/source, mob/crosser, bitecount)
	...
	popper.visible_message(span_danger("[popper] steps on \the [source], popping the bag!"), span_danger("You step on \the [source], popping the bag!"), span_danger("Você ouve uma rachadura afiada!"), COMBAT_MESSAGE_RANGE)
	INVOKE_ASYNC(src, PROC_REF(async_generate_trash), source)
	qdel(source)
