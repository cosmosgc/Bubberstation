...
/obj/item/tattoo_kit
	name = "tattoo kit"
	desc = "Um kit com todas as ferramentas necessárias para perder uma aposta, ou tomar decisões incrivelmente indeléveis."
...
/obj/item/tattoo_kit/item_interaction(mob/living/user, obj/item/toner/ink_cart, list/modifiers)
	...
	if(added_amount == 0)
		balloon_alert(user, "Não sobrou nada!")
	...
	qdel(ink_cart)
	balloon_alert(user, "Tinta de tatuagem adicionada")
...
/obj/item/tattoo_kit/attack(mob/living/tattoo_holder, mob/living/tattoo_artist, list/modifiers, list/attack_modifiers)
	...
	if(!tattoo_artist.mind || tattoo_artist.combat_mode)
		return
	if(!uses)
		balloon_alert(tattoo_artist, "Não tem tinta suficiente!")
		return
	if(!length(tattoo_artist.mind.memories))
		balloon_alert(tattoo_artist, "Nada memorável para gravar!")
		return
	...
	if(!tattoo_target)
		balloon_alert(tattoo_artist, "Sem membro para tatuagem!")
		return
	if(HAS_TRAIT_FROM(tattoo_target, TRAIT_NOT_ENGRAVABLE, ENGRAVED_TRAIT))
		balloon_alert(tattoo_artist, "Parte do corpo já tatuada!")
		return
	if(HAS_TRAIT(tattoo_target, TRAIT_NOT_ENGRAVABLE))
		balloon_alert(tattoo_artist, "Parte do corpo não pode ser tatuada!")
		return
