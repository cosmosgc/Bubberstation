...
/datum/proximity_monitor/advanced/heretic_arena/New(atom/_host, range, _ignore_if_not_on_turf)
	...
	for(var/mob/living/carbon/human/human_in_range in things_in_range)
		...
		if(!IS_HERETIC(human_in_range))
			...
			human_in_range.balloon_alert(human_in_range, "Comecem a mata!")
...
/datum/antagonist/heretic_arena_participant/forge_objectives()
	...
	fight_to_escape.explanation_text = "Escape is impossible. The only way out is to defeat another participant in this battle to the death. 		A weapon has been bestowed unto you, granting you a fighting chance, it would be quite a shame were you to attempt to break it."
