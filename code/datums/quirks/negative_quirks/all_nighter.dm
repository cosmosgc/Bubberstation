...
/datum/quirk/all_nighter
	name = "All Nighter"
	desc = "Você não dormiu ontem à noite, e as pessoas podem dizer! Você estará constantemente de mau humor e terá uma tendência a dormir mais. Estimulantes ou uma soneca podem ajudar."
	icon = FA_ICON_MOON
	value = -4
	mob_trait = TRAIT_HEAVY_SLEEPER
	gain_text = span_danger("Você se sente exausto.")
	lose_text = span_notice("Você se sente bem descansado.")
	medical_record_text = "O paciente parece estar sofrendo de privação de sono."
	hardcore_value = 2
...
/datum/quirk/all_nighter/process(seconds_per_tick)
	...
	if(!all_nighter && !happy_camper)
		quirk_holder.add_mood_event("all_nighter", /datum/mood_event/all_nighter)
		to_chat(quirk_holder, span_danger("Você começa a se sentir cansado novamente."))
