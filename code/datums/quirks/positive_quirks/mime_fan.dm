/datum/quirk/item_quirk/mime_fan
	name = "Mime Fan"
	desc = "Você é fã de mímicas e ganha um impulso de humor usando seu pino mímico."
	icon = FA_ICON_THUMBTACK
	value = 2
	mob_trait = TRAIT_MIME_FAN
	gain_text = span_notice("Você é um grande fã do Mime.")
	lose_text = span_danger("O mímico não parece tão bom.")
	medical_record_text = "O paciente relata ser um grande fã de mímicas."
	mail_goodies = list(
