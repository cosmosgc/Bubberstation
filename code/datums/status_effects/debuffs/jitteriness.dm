...
/datum/status_effect/jitter/get_examine_text()
	switch(duration)
		if(5 MINUTES to INFINITY)
			return span_boldwarning("[owner.p_They()] [owner.p_are()] Convulsão violenta!")
		if(3 MINUTES to 5 MINUTES)
			return span_warning("[owner.p_They()] [owner.p_are()] extremamente nervoso.")
		if(1 MINUTES to 3 MINUTES)
			return span_warning("[owner.p_They()] [owner.p_are()] Estremecendo levemente.")
