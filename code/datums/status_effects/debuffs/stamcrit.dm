...
/datum/status_effect/incapacitating/stamcrit/on_apply()
	...
	if(owner.stat == CONSCIOUS)
		to_chat(owner, span_notice("Está exausta demais para continuar..."))
...
/datum/status_effect/incapacitating/stamcrit/proc/update_diminishing_return(datum/source, type, amount, forced)
	...
	if(COOLDOWN_FINISHED(src, warn_cd) && owner.stat == CONSCIOUS)
		to_chat(owner, span_notice("Você começa a se recuperar da exaustão!"))
		owner.visible_message(span_warning("[owner] Começa a se recuperar da exaustão!"), ignored_mobs = owner)
