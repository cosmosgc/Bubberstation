...
/datum/voice_of_god_command/hallucinate/execute(list/listeners, mob/living/user, power_multiplier = 1, message)
	for(var/mob/living/target in listeners)
		target.cause_hallucination( 			get_random_valid_hallucination_subtype(/datum/hallucination/delusion/preset), 			"voice of god", 			duration = 15 SECONDS * power_multiplier, 			affects_us = FALSE, 			affects_others = TRUE, 			skip_nearby = FALSE, 		)
