...
/datum/vote/restart_vote
	...
	default_message = "Vote para reiniciar a rodada. Só funciona se não houver administradores não-AFK online."
...
/datum/vote/restart_vote/proc/async_alert_about_admins(mob/vote_creator)
	set waitfor = FALSE
	tgui_alert(vote_creator, "Nota: independente dos resultados desta votação, a rodada não vai reiniciar automaticamente porque um administrador ativo está online.")
...
/datum/vote/restart_vote/finalize_vote(winning_option)
	...
	if(winning_option == CHOICE_RESTART)
		if(admins_present())
			to_chat(world, span_boldannounce("Nota: uma votação de reinício não reiniciará o servidor automaticamente porque há administradores ativos ligados."))
		...
		if(!isnull(SSmap_vote.next_map_config))
			...
			send_to_playing_players(span_boldannounce("O próximo mapa foi reiniciado devido ao sucesso da votação."))
