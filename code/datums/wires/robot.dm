	...
	switch(wire)
		...
		if(WIRE_CAMERA) // Pulse to disable the camera.
			if(!QDELETED(R.builtInCamera) && !R.scrambledcodes)
				...
				R.visible_message(span_notice("[R] As lentes da câmera se concentram alto."), span_notice("Sua lente da câmera se concentra alto."))
		...
		if(WIRE_LAWSYNC) // Forces a law update if possible.
			if(R.lawupdate)
				R.visible_message(span_notice("[R] Com cuidado."), span_notice("Protocolo LawSync Ativado."))
		...
		if(WIRE_RESET_MODEL)
			if(R.has_model())
				R.visible_message(span_notice("[R] O modelo de servose contorce."), span_notice("Seu modelo de exibição pisca."))
