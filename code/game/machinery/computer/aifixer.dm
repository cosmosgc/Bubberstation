/obj/machinery/computer/aifixer
	...
	desc = "Usado com cartões intelli contendo IA não funcionais para restaurá-los."
	...
	switch(action)
		if("PRG_beginReconstruction")
			if(occupier?.health < 100)
				to_chat(usr, span_notice("Reconstrução em andamento. Isso levará vários minutos."))
...
/obj/machinery/computer/aifixer/proc/Fix()
	...
	if(occupier.health >= 0 && occupier.stat == DEAD)
		...
		if(!occupier.radio_enabled)
			...
			to_chat(occupier, span_warning("Seu Transceptor Subespacial foi ativado!"))
...
/obj/machinery/computer/aifixer/transfer_ai(interaction, mob/user, mob/living/silicon/ai/AI, obj/item/aicard/card)
	...
	if(interaction == AI_TRANS_FROM_CARD)
		...
		to_chat(AI, span_alert("Você foi carregado para um terminal estacionário. Infelizmente, não há acesso remoto daqui."))
		...
		if(occupier && !restoring)
			to_chat(occupier, span_notice("Você foi baixado para um dispositivo de armazenamento móvel. Ainda sem acesso remoto."))
			...
			to_chat(user, span_alert("Reconstrução em andamento."))
		else if (!occupier)
			to_chat(user, span_alert("Incapaz de localizar inteligência artificial."))
