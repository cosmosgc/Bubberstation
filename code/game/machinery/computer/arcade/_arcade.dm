/obj/machinery/computer/arcade
	...
	desc = "Este armário não tem jogos instalados, e na verdade, não deveria existir.\
Informe a localização desta máquina para sua dieta local."
	icon_state = "arcade"
	icon_keyboard = null
...
/obj/machinery/computer/arcade/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if(istype(tool, /obj/item/stack/arcadeticket))
		...
		if(!tickets.use(2))
			balloon_alert(user, "Preciso de 2 ingressos!")
...
/obj/machinery/computer/arcade/proc/prizevend(mob/living/user, prizes = 1)
	if(user.mind?.get_skill_level(/datum/skill/gaming) >= SKILL_LEVEL_LEGENDARY && HAS_TRAIT(user, TRAIT_GAMERGOD))
		...
		span_notice("Você ouve um barulho de botões sendo pressionados."))
	...
	for(var/i in 1 to prizes)
		...
		if(prob(0.0001)) //1 in a million
			...
			visible_message(span_notice("[src] dispenses.. woah, a gun! Way past cool."), span_notice("Você ouve um sino e um tiro."))
		...
		visible_message(span_notice("[src] dispenses [the_prize]!"), span_notice("Você ouve um sino e um barulho."))
