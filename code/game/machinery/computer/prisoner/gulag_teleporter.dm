...
/obj/machinery/computer/prisoner/gulag_teleporter_computer
	...
	desc = "Costumava mandar criminosos para o Campo de Trabalho."
	...
	switch(action)
		...
		if("toggle_open")
			if(teleporter.locked)
				to_chat(usr, span_alert("O teletransporte deve ser desbloqueado primeiro."))
				return
			teleporter.toggle_open()
			return TRUE
		if("teleporter_lock")
			if(teleporter.state_open)
				to_chat(usr, span_alert("O teletransporte deve ser fechado primeiro."))
				return
...
/obj/machinery/computer/prisoner/gulag_teleporter_computer/proc/teleport(mob/user)
	...
	to_chat(prisoner, span_warning("O teletransporte te deixa um pouco tonto."))
