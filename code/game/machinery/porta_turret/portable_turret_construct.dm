...
/obj/machinery/porta_turret_construct/examine(mob/user)
	...
	switch(build_step)
		if(PTURRET_UNSECURED)
			. += span_notice("Os parafusos externos são<b>sem torção</b>E o quadrado pode ser<i>Invadido</i>Separados.")
		if(PTURRET_BOLTED)
			. += span_notice("O quadrado requer<b>metal</b>para sua armadura interna, os parafusos externos são<i>Estrangulada.</i>Nenhum lugar.")
		...
		if(PTURRET_INTERNAL_ARMOUR_ON)
			. += span_notice("Uma torre reque...<b>Arma baseada em energia.</b>Para funcionar, a armadura é protegida por<i>Parafusos</i>.")
		if(PTURRET_GUN_EQUIPPED)
			. += span_notice("Uma torre reque...<b>Sensor de proximidade</b>para funcionar. A arma de energia poderia<i>ser removido.</i>.")
		if(PTURRET_SENSORS_ON)
			. += span_notice("A escotilha de acesso da torre é<b>\"Desenroso\".</b>O sensor de proximidade poderia<i>ser removido.</i>.")
		if(PTURRET_CLOSED)
			. += span_notice("Um torre exige<b>metal</b>para sua armadura externa, a escotilha de acesso pode ser<i>\"Desenroso\".</i>.")
