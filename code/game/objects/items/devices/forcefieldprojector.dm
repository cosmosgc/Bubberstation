/obj/item/forcefield_projector
	...
	desc = "Um dispositivo experimental que pode criar vários campos de força à distância."
...
/obj/item/forcefield_projector/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	if(found_field)
		to_chat(user, span_warning("Já há um campo de força naquele local!"))
		return ITEM_INTERACT_BLOCKING
	if(T.density)
		return ITEM_INTERACT_BLOCKING
	...
	user.visible_message(span_warning("[user] projects a forcefield!"),span_notice("Você projeta um campo de força."))
...
/obj/structure/projected_forcefield
	name = "forcefield"
	desc = "Uma barreira brilhante, gerada por um projetor próximo. Pode estar sobrecarregado se bater várias vezes."
