...
/obj/item/batterer/attack_self(mob/living/carbon/user, flag = 0, emp = 0)
	...
	to_chat(user, span_notice("Você aciona.[src]."))
	times_used += 1
	if(times_used >= max_uses)
...
/obj/item/shadowcloak/weaker
	name = "stealth belt"
	desc = "Faz você quase invisível a olho nu por um curto período de tempo. Dura indefinidamente na escuridão, mas não recarregará a menos que inativo."
...
/obj/item/jammer/attack_self_secondary(mob/user, modifiers)
	...
	user.balloon_alert(user, "[active ? "deactivated" : "activated"]O bloqueador")
	active = !active
	if(active)
...
/obj/item/jammer/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	to_chat(user, span_notice("Você libera uma onda disruptora direcionada, desabilitando todos os dispositivos de rádio.[interacting_with]."))
...
/obj/item/pen/penbang/on_transform(obj/item/source, mob/user, active)
	...
	if(user)
		to_chat(user, span_warning("Você prepara o penbang![capitalize(DisplayTimeText(det_time))]!"))
