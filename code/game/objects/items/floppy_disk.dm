...
/obj/item/disk/examine(mob/user)
	. = ..()
	. += span_notice("A aba de proteção de escrita está definida para[span_bold("[read_only ? "protected" : "unprotected"]")].")
...
/obj/item/disk/attack_self(mob/user)
	if(read_only_locked)
		...
		return
	read_only = !read_only
	to_chat(user, span_notice("Você vira a aba de proteção de escrita para[span_bold("[read_only ? "protected" : "unprotected"]")]."))
...
/obj/item/disk_stack/examine(mob/user)
	. = ..()
	. += span_notice("Há[span_bold("[length(stacked_disks)]")]Discos na pilha.")
...
/obj/item/disk_stack/proc/spread()
	...
	visible_message(span_warning("Uma pilha desmorona!"))
	qdel(src)
