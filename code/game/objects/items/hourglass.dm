...
/obj/item/hourglass
	name = "hourglass"
	desc = "Nanotrasen patenteou a ampulheta invariante da gravidade. Garantido para fluir perfeitamente sob quaisquer condições."
...
/obj/item/hourglass/proc/toggle(mob/user)
	if(!timing_id)
		...
		to_chat(user,span_notice("Pare com isso.[src].")) //Sand magically flows back because that's more convinient to use.
		stop()
