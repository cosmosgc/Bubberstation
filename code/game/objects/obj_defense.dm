...
/obj/bullet_act(obj/projectile/hitting_projectile, def_zone, piercing_hit = FALSE, blocked = null)
	...
	if(hitting_projectile.suppressed != SUPPRESSED_VERY)
		visible_message(
			span_danger("[src] é atingido por\a [hitting_projectile][damage_sustained ? "" : ", [no_damage_feedback]"]!"),
...
/obj/attack_hulk(mob/living/carbon/human/user)
	...
	user.visible_message(span_danger("[user] Esmaga.[src][damage ? "" : ", [no_damage_feedback]"]!"), span_danger("Você quebra [src][damage ? "" : ", [no_damage_feedback]"]!"), null, COMBAT_MESSAGE_RANGE)
	return TRUE
