...
/obj/structure/hivebot_beacon/Initialize(mapload)
	. = ..()
	do_smoke(2, src, loc)
	visible_message(span_bolddanger("[src]Deformações!"))
...
/obj/structure/hivebot_beacon/proc/warpbots()
	...
	visible_message(span_danger("[src]Liga!"))
	while(bot_amt > 0)
		bot_amt--
		switch(bot_type)
			if("norm")
	...
	sleep(10 SECONDS)
	visible_message(span_bolddanger("[src]dobras para fora!"))
	playsound(src.loc, 'sound/effects/empulse.ogg', 25, TRUE)
	qdel(src)
	return
