...
/obj/structure/secure_safe/caps_spare
	...
	desc = "Em caso de emergência, não quebre vidro. Todos os Capitães e Capitães Interagentes têm códigos para acessar este cofre. É feito do mesmo material que a Caixa Negra da estação e foi projetado para resistir a todas as armas convencionais. Parece haver uma pequena quantidade de corrosão superficial. Não parece que suportaria uma explosão. Devido ao material caro, foi feito incrivelmente pequeno para cortar curvas, deixando apenas espaço suficiente para caber algo tão fino como um cartão de identidade."
...
/obj/structure/secure_safe/caps_spare/Initialize(mapload)
	...
	AddComponent(/datum/component/lockable_storage, 		lock_code = SSid_access.spare_id_safe_code, 		can_hack_open = FALSE, 	)
