...
/obj/structure/table
	name = "table"
	desc = "Um pedaço quadrado de ferro sobre quatro pernas de metal. Não pode se mover."
...
/obj/structure/table/Initialize(mapload, obj/structure/table_frame/frame_used, obj/item/stack/stack_used)
	...
	if(can_flip)
		AddElement( 			/datum/element/contextual_screentip_bare_hands, 			rmb_text = "Flip", 		)
...
/obj/structure/table/examine(mob/user)
	. = ..()
	if(is_flipped)
		. += span_notice("Ele foi virado de lado!")
	. += deconstruction_hints(user)

/obj/structure/table/proc/deconstruction_hints(mob/user)
	return span_notice("O topo é<b>Está ferrado.</b>Mas o principal...<b>parafusos</b>são também visíveis.")
...
/obj/structure/table/rolling
	name = "Rolling table"
	desc = "Uma marca NT\"Rolly poly\"Tábua rolando. Pode e vai se mover."
	anchored = FALSE
...
/obj/structure/table/glass
	name = "glass table"
	desc = "O que eu disse sobre encostar nas mesas de vidro? Agora precisa de cirurgia."
...
/obj/structure/table/glass/plasmaglass
	...
	desc = "Alguém achou uma boa ideia."
...
/obj/structure/table/wood
	name = "wooden table"
	desc = "Não aplique fogo nisso. Dizem que queima facilmente."
...
/obj/structure/table/wood/poker //No specialties, Just a mapping object.
	name = "gambling table"
	desc = "Uma mesa desordenada para negócios sujos em lugares sujos."
...
/obj/structure/table/wood/fancy
	name = "fancy table"
	desc = "Um quadro padrão de mesa de metal coberto com um pano surpreendentemente elegante e estampado."
...
/obj/structure/table/reinforced
	...
	desc = "Uma versão reforçada da mesa de quatro pernas."
...
/obj/structure/table/reinforced/deconstruction_hints(mob/user)
	if(deconstruction_ready)
		return span_notice("A capa de cima foi<i>soldado</i>solto e o quadro principal<b>parafusos</b>estão expostos.")
	else
		return span_notice("A capa de cima está firme.<b>soldado</b>Vamos.")
...
/obj/structure/table/reinforced/welder_act_secondary(mob/living/user, obj/item/tool)
	if(tool.tool_start_check(user, amount = 0))
		...
		if(deconstruction_ready)
			to_chat(user, span_notice("Você começa a fortalecer a mesa reforçada..."))
			if (tool.use_tool(src, user, 50, volume = 50))
				to_chat(user, span_notice("Você fortalece a mesa."))
				deconstruction_ready = FALSE
				return ITEM_INTERACT_SUCCESS
		else
			to_chat(user, span_notice("Você começa a enfraquecer a mesa reforçada..."))
			if (tool.use_tool(src, user, 50, volume = 50))
				to_chat(user, span_notice("Você enfraquece a mesa."))
...
/obj/structure/table/bronze
	name = "bronze table"
	desc = "Uma mesa sólida feita de bronze."
...
/obj/structure/table/reinforced/rglass
	...
	desc = "Uma versão reforçada da mesa de vidro."
...
/obj/structure/table/reinforced/plasmarglass
	...
	desc = "Uma versão reforçada da mesa de vidro de plasma."
...
/obj/structure/table/reinforced/titaniumglass
	...
	desc = "Uma mesa de vidro reforçada com titânio, com uma nova camada de tinta branca NT."
...
/obj/structure/table/reinforced/plastitaniumglass
	...
	desc = "Uma mesa de titânio reforçado com sílica-plasma composto. Tão durável quanto parece."
...
/obj/structure/table/optable//SKYRAT EDIT - ICON OVERRIDEN BY AESTHETICS - SEE MODULE
	...
	desc = "Usado para procedimentos médicos avançados."
...
/obj/structure/table/optable/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	if (istype(tool, /obj/item/clothing/mask/breath))
		if (breath_mask && breath_mask != tool)
			balloon_alert(user, "A máscara já está presa!")
			return ITEM_INTERACT_BLOCKING
		...
		balloon_alert(user, "máscara anexada")
	...
	if (air_tank)
		balloon_alert(user, "O tanque já está preso!")
		return ITEM_INTERACT_BLOCKING
	...
	air_tank = as_tank
	balloon_alert(user, "Tanque ligado.")
	playsound(src, 'sound/machines/click.ogg', 50, TRUE)
	update_appearance()
	return ITEM_INTERACT_SUCCESS
...
/obj/structure/table/optable/screwdriver_act(mob/living/user, obj/item/tool)
	...
	balloon_alert(user, "Máscara separada")
	UnregisterSignal(breath_mask, list(COMSIG_MOVABLE_MOVED, COMSIG_ITEM_DROPPED))
...
/obj/structure/table/optable/wrench_act(mob/living/user, obj/item/tool)
	if (!air_tank)
		return NONE
	balloon_alert(user, "Desamarrando o tanque...")
	if (!tool.use_tool(src, user, 3 SECONDS))
		return ITEM_INTERACT_BLOCKING
	air_tank.forceMove(drop_location())
	tool.play_tool_sound(src, 50)
	balloon_alert(user, "Tanque separado.")
...
/obj/structure/table/optable/proc/detach_mask(mob/living/user)
	...
	if (!breath_mask)
		balloon_alert(user, "Sem máscara!")
		return TRUE
...
/obj/structure/table/optable/mouse_drop_dragged(atom/over, mob/living/user, src_location, over_location, params)
	...
	if (!air_tank)
		balloon_alert(user, "Nenhum tanque preso!")
		return
...
/obj/structure/rack
	name = "rack"
	desc = "Diferente da versão da Idade Média."
...
/obj/item/rack_parts/attack_self(mob/user)
	if(building)
		return
	building = TRUE
	to_chat(user, span_notice("Você começa a construir uma prateleira..."))
