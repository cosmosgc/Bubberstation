...
/turf/closed/wall/r_wall/deconstruction_hints(mob/user)
	switch(d_state)
		...
		if(SUPPORT_LINES)
			return span_notice("O exterior<i>Grelha</i>foi cortado, e as linhas de apoio são<b>Está ferrado.</b>Seguramente para a cobertura externa.")
		if(COVER)
			return span_notice("As linhas de apoio foram<i>Desenroscado</i>E a capa de metal é<b>soldado</b>firmemente no lugar.")
		if(CUT_COVER)
			return span_notice("A capa de metal tem sido<i>Cortado através</i>, e é<b>conectados vagamente</b>Para a viga.")
		if(ANCHOR_BOLTS)
			return span_notice("A capa externa foi<i>Foi embora.</i>, e os parafusos ancorando as hastes de apoio são<b>Estrangulada.</b>No lugar.")
		if(SUPPORT_RODS)
			return span_notice("Os parafusos ancorando as hastes de apoio foram<i>afrouxado.</i>, mas ainda são<b>soldado</b>firmemente para a viga.")
		if(SHEATH)
			return span_notice("As barras de apoio foram<i>Cortado através</i>, e a bainha exterior é<b>conectados vagamente</b>Para a viga.")
		...
		if(COVER)
			...
			if(W.tool_behaviour == TOOL_SCREWDRIVER)
				...
				if(W.use_tool(src, user, 40, volume=100))
					...
					d_state = SUPPORT_LINES
					update_appearance()
					to_chat(user, span_notice("As linhas de apoio foram seguras."))
				return TRUE
