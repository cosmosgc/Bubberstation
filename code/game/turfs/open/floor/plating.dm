...
/turf/open/floor/plating/attackby(obj/item/C, mob/user, list/modifiers)
	...
	if(istype(C, /obj/item/stack/rods) && attachment_holes)
		...
		if (R.get_amount() < 2)
			...
			to_chat(user, span_notice("Você começa a reforçar o chão..."))
			if(do_after(user, 3 SECONDS, target = src))
				if (R.get_amount() >= 2 && !istype(src, /turf/open/floor/engine))
					place_on_top(/turf/open/floor/engine, flags = CHANGETURF_INHERIT_AIR)
					playsound(src, 'sound/items/deconstruct.ogg', 80, TRUE)
					R.use(2)
					to_chat(user, span_notice("Você reforça o chão."))
				return
		...
		if((broken || burnt) && C.use_tool(src, user, 0, volume=80))
			to_chat(user, span_danger("Você conserta alguns amassados no revestimento quebrado."))
		...
		if (sheet.get_amount() < 1)
			...
			playsound(src, 'sound/items/deconstruct.ogg', 80, TRUE)
			sheet.use(1)
			to_chat(user, span_notice("Você isola o chão."))
...
/turf/open/floor/plating/foam/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(lattice)
		qdel(lattice)
	to_chat(user, span_notice("Você reforça o revestimento espumado com tiling."))
