...
/proc/admin_proc()
	if(!check_rights(R_ADMIN))
		return
	to_chat(world, "Você tem direitos suficientes!", confidential = TRUE)
...
/proc/check_rights(rights_required, show_msg=1)
	if(usr?.client)
		if (check_rights_for(usr.client, rights_required))
			...
			if(show_msg)
				to_chat(usr, "<font color='red'>Erro: você não tem direitos suficientes para fazer isso. Você precisa de uma das seguintes bandeiras:[rights2text(rights_required," ")].</font>", confidential = TRUE)
	return FALSE
