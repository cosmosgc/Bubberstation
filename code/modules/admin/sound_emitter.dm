...
/obj/effect/sound_emitter/examine(mob/user)
	...
	. += span_boldnotice("Modo:</span> [motus_operandi]")
	. += span_boldnotice("Distância:</span> [emitter_range]")
...
/obj/effect/sound_emitter/click_alt(mob/user)
	...
	activate(user)
	to_chat(user, span_notice("Emissário de som ativado."), confidential = TRUE)
