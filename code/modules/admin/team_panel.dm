...
/datum/admins/proc/admin_create_team(mob/user)
	var/team_name = stripped_input(user,"Nome cumprir pena?")
	if(!team_name)
		return
...
/datum/team/proc/admin_rename(mob/user)
	var/old_name = name
	var/team_name = stripped_input(user,"Novo nome de equipamento?","Equipe Renomear",old_name)
	if(!team_name)
		return
	name = team_name
...
/datum/team/proc/admin_communicate(mob/user)
	var/message = input(user,"Mensagem para um equipamento?","Mensagem da Equipe") as text|null
	if(!message)
		return
