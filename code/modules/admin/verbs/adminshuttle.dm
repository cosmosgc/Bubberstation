...
ADMIN_VERB(call_shuttle, R_ADMIN, "Call Shuttle", "Force a shuttle call with additional modifiers.", ADMIN_CATEGORY_SHUTTLE)
	...
	var/confirm = tgui_alert(user, "Temcereza?", "Confirm", list("Yes", "Yes (No Recall)", "No"))
	switch(confirm)
		if(null, "No")
			return
		if("Yes (No Recall)")
...
ADMIN_VERB(cancel_shuttle, R_ADMIN, "Cancel Shuttle", "Recall the shuttle, regardless of circumstances.", ADMIN_CATEGORY_SHUTTLE)
	...
	if(tgui_alert(user, "Temcereza?", "Confirm Shuttle Cancellation", list("Yes", "No")) != "Yes")
		return
...
ADMIN_VERB(disable_shuttle, R_ADMIN, "Disable Shuttle", "Those fuckers aren't getting out.", ADMIN_CATEGORY_SHUTTLE)
	...
	if(tgui_alert(user, "Temcereza?", "Confirm", list("Yes", "No")) != "Yes")
		return
...
/obj/docking_port/mobile/proc/admin_fly_shuttle(mob/user)
	...
	switch(selection)
		...
		if("Delete Shuttle")
			if(tgui_alert(user, "Realmente deletar[name || shuttle_id]?", "Delete Shuttle", list("Cancel", "Really!")) != "Really!")
				return
			jumpToNullSpace()

		if("Into The Sunset (delete & greentext 'escape')")
			if(tgui_alert(user, "Realmente deletar[name || shuttle_id]e objetivos de fuga de texto verde?", "Delete Shuttle", list("Cancel", "Really!")) != "Really!")
				return
			intoTheSunset()
