...
ADMIN_VERB(toggle_respawn, R_SERVER, "Toggle Respawn", "Toggle the ability to respawn.", ADMIN_CATEGORY_SERVER)
	...
	if(new_state == -1)
		to_chat(user, span_warning("A configuração para respawn está definida incorretamente, por favor, reclame para seu servidor mais próximo (ou conserte você mesmo).\
Enquanto isso, Respawn foi definido para\"Fora.\"."))
		new_state = RESPAWN_FLAG_DISABLED
		new_state_text = "Disabled"
