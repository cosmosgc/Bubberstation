...
/client/proc/vv_parse_text(O, new_var)
	if(O && findtext(new_var,"\["))
		var/process_vars = tgui_alert(usr,"\[]Detectado em cordas, processo como variáveis?","Process Variables?",list("Yes","No"))
		if(process_vars == "Yes")
...
/client/proc/mod_list(list/L, atom/O, original_name, objectvar, index, autodetect_class = FALSE)
	...
	to_chat(src, "Variável contém:[variable]", confidential = TRUE)
	...
	if(default == VV_NUM)
		...
		if(dir_text)
			to_chat(usr, "Se uma direção é:[dir_text]", confidential = TRUE)
...
/client/proc/modify_variables(atom/O, param_var_name = null, autodetect_class = 0)
	...
	if(param_var_name)
		if(!(param_var_name in O.vars))
			to_chat(src, "Uma variável com este nome ([param_var_name] Não existe neste dado.[O])", confidential = TRUE)
			return
