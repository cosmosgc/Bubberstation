...
//~CARN: for renaming mobs (updates their name, real_name, mind.name, their ID/PDA and datacore records).
	if(href_list["rename"])
		...
		var/new_name = stripped_input(usr,"Como você gostaria de chamar essa multidão?","Introdução um nome",M.real_name,MAX_NAME_LEN)
		...
		var/amount = input("Lidar com os danos da máfia? (Valores negativos aqui curar)","Ajustar.[Text]Perda.",0) as num|null
