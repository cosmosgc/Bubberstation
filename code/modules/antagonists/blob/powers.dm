...
/mob/eye/blob/proc/create_special(price, blobstrain, min_separation, needs_node, turf/tile)
	...
	if(!istype(blob, /obj/structure/blob/normal))
		to_chat(src, span_warning("Incapaz de usar essa bolha, encontre uma normal."))
		balloon_alert(src, "Preciso de bolhas normais!")
		return FALSE
	if(needs_node)
...
/mob/eye/blob/proc/remove_blob(turf/tile)
	...
	if(blob.point_return < 0)
		to_chat(src, span_warning("Incapaz de remover esta bolha."))
		return FALSE
