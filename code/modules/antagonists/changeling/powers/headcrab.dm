...
/datum/action/changeling/headcrab
	name = "Last Resort"
	desc = "Sacrificamos nosso corpo atual em um momento de necessidade, expandindo-se violentamente para romper obstáculos e reformando como um headslug. Custa 20 produtos químicos."
...
/datum/action/changeling/headcrab/sting_action(mob/living/user)
	set waitfor = FALSE
	var/confirm = tgui_alert(user, "Temos certeza de que queremos destruir nosso corpo e criar um headslug?", "Last Resort", list("Yes", "No"))
...
/datum/action/changeling/headcrab/proc/gore_explosion(mob/living/user)
	...
	for(var/mob/living/blinded in view(LAST_RESORT_BLIND_RANGE, user))
		if(blinded == user)
			continue
		blinded.visible_message(span_danger("[blinded] is splattered with blood!"), span_userdanger("Você está cheio de sangue!"))
		...
		if(ishuman(blinded))
			...
			if(!eyes || blinded_human.is_blind())
				continue
			to_chat(blinded_human, span_userdanger("Você está cego por uma chuva de sangue!"))
		...
		if(issilicon(blinded))
			...
			to_chat(blinded_silicon, span_userdanger("Seus sensores estão desativados por um banho de sangue!"))
...
/datum/action/changeling/headcrab/proc/spawn_headcrab(datum/mind/stored_mind, turf/spawn_location, list/organs)
	...
	to_chat(crab, span_warning("Saímos dos restos do nosso antigo corpo em um banho de sangue!"))
	active = FALSE
