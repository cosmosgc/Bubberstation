...
/obj/effect/rune
	name = "rune"
	desc = "Uma estranha coleção de símbolos desenhados no que parece ser sangue."
	anchored = TRUE
...
/obj/effect/rune/attack_animal(mob/living/user, list/modifiers)
	...
	if(HAS_TRAIT(user, TRAIT_ANGELIC))
		to_chat(user, span_warning("Você purga a runa!"))
		qdel(src)
...
/obj/effect/rune/proc/fail_invoke()
	...
	visible_message(span_warning("As marcas pulsam com um pequeno flash de luz vermelha, e depois ficam escuras."))
	var/oldcolor = color
	color = rgb(255, 0, 0)
...
/obj/effect/rune/malformed
	...
	cultist_desc = "Uma runa sem sentido escrita em palavrões. Nada de bom pode vir de invocar isso."
...
/obj/effect/rune/convert
	cultist_name = "Offer"
	cultist_desc = "Oferece um não-cultista acima dele para Nar'Sie, ou convertendo-os ou sacrificando-os."
	req_cultists_text = "2 para conversão, 3 para sacrifícios vivos e alvos de sacrifício."
...
/obj/effect/rune/convert/proc/do_convert(mob/living/convertee, list/invokers, datum/team/cult/cult_team)
	...
	to_chat(convertee, span_cult_bold_italic("Seu pulso sanguíneo. Sua cabeça bate. O mundo fica vermelho. Tudo de uma vez você está ciente de uma horrível, horrível, verdade. O véu da realidade foi arrancado e algo maligno se enraiza."))
	to_chat(convertee, span_cult_bold_italic("Ajude seus novos compatriotas em seus negócios obscuros. Seu objetivo é deles, e deles é seu. Você serve o Geômetro acima de tudo. Traga de volta."))
...
/obj/effect/rune/convert/proc/do_sacrifice(mob/living/sacrificial, list/invokers, datum/team/cult/cult_team)
	...
	if(do_message)
		for(var/invoker in invokers)
			if(target_sac)
				to_chat(invoker, span_cult_large("\"Sim! Este é o que eu desejo! Você fez bem.\""))
				continue
			if(ishuman(sacrificial) || iscyborg(sacrificial))
				to_chat(invoker, span_cult_large("\"Aceito este sacrifício.\""))
			else
				to_chat(invoker, span_cult_large("\"Eu aceito este pequeno sacrifício.\""))
...
/obj/effect/rune/empower
	...
	cultist_desc = "permite que os cultistas preparem maiores quantidades de magia de sangue a um custo muito menor."
...
/obj/effect/rune/teleport
	...
	cultist_desc = "dobra tudo acima dele para outra runa de teletransporte escolhida."
...
/obj/effect/rune/teleport/invoke(list/invokers)
	...
	if(target.is_blocked_turf(TRUE))
		to_chat(user, span_warning("A runa do alvo está bloqueada. Tentar se teletransportar para ele seria muito imprudente."))
	...
	if(movedsomething)
		...
		if(moveuserlater)
			if(do_teleport(user, target, channel = TELEPORT_CHANNEL_CULT))
				movesuccess = TRUE
		if(movesuccess)
			visible_message(span_warning("Há uma rajada de ar, e tudo acima da runa desaparece!"), null, "<i>Você ouve uma rachadura afiada.</i>")
			to_chat(user, span_cult("Você.[moveuserlater ? "r vision blurs, and you suddenly appear somewhere else":" send everything above the rune away"]."))
		else
			to_chat(user, span_cult("Você.[moveuserlater ? "r vision blurs briefly, but nothing happens":" try send everything above the rune away, but the teleportation fails"]."))
		...
		if(movesuccess)
			target.visible_message(span_warning("Há uma explosão de ar como algo aparece acima da runa!"), null, "<i>Você ouve um boom.</i>")
	else
		fail_invoke()
...
/obj/effect/rune/narsie
	...
	cultist_desc = "Rasga barreiras dimensionais, chamando o Geômetro. Requer 9 invocadores."
...
/obj/effect/rune/narsie/invoke(list/invokers)
	...
	if(!(place in summon_objective.summon_spots))
		to_chat(user, span_cult_large("O Geômetro só pode ser convocado onde o véu está fraco.[english_list(summon_objective.summon_spots)]!"))
		return
	if(locate(/obj/narsie) in SSpoints_of_interest.narsies)
		for(var/invoker in invokers)
			to_chat(invoker, span_warning("Nar'Sie já está neste avião!"))
...
/obj/effect/rune/raise_dead
	cultist_name = "Revive"
	cultist_desc = "requer um culto morto, desmiolado ou inativo colocado sobre a runa. Para cada três corpos sacrificados ao patrono das trevas, um corpo será consertado e sua mente acordada."
...
/obj/effect/rune/raise_dead/invoke(list/invokers)
	...
	if(!length(potential_revive_mobs))
		to_chat(user, span_cult_italic("Não há cultistas mortos na runa!"))
	...
	if(!mob_to_revive.client || mob_to_revive.client.is_afk())
		set waitfor = FALSE
		var/mob/chosen_one = SSpolling.poll_ghosts_for_target("Do you want to play as [span_danger(mob_to_revive.real_name)], an [span_notice("inactive blood cultist")]?", check_jobban = ROLE_CULTIST, role = ROLE_CULTIST, poll_time = 5 SECONDS, checked_target = mob_to_revive, alert_pic = mob_to_revive, role_name_text = "cultista inativo")
		if(chosen_one)
			to_chat(mob_to_revive.mind, "Sua forma física foi tomada por outra alma devido à sua inatividade! Uma ajuda se quiser recuperar sua forma.")
...
/obj/effect/rune/raise_dead/proc/validness_checks(mob/living/target_mob, mob/living/user)
	...
	if(!(target_mob in loc))
		to_chat(user, span_cult_italic("O cultista para reviver foi movido!"))
...
/obj/effect/rune/wall
	...
	cultist_desc = "Quando invocado, faz uma parede invisível temporária para bloquear a passagem. Pode ser invocado novamente para reverter isso."
...
/obj/effect/rune/summon
	...
	cultist_desc = "Convoca um único cultista para a runa. Requer 2 invocadores."
...
/obj/effect/rune/summon/invoke(list/invokers)
	...
	if(length(cultists) <= 1)
		to_chat(user, span_warning("Não há cultistas para convocar!"))
		fail_invoke()
		return
	...
	if(isnull(cultist_to_summon))
		to_chat(user, span_cult_italic("Você precisa de um alvo de invocação!"))
...
/obj/effect/rune/blood_boil
	...
	cultist_desc = "ferve o sangue de não-crentes que podem ver a runa, lidando rapidamente com quantidades extremas de danos. Requer 3 invocadores."
	...
	for(var/mob/living/target in viewers(T))
		if(!IS_CULTIST(target) && CAN_HAVE_BLOOD(target))
			if(target.can_block_magic(charge_cost = 0))
				continue
			to_chat(target, span_cult_large("Seu sangue ferve em suas veias!"))
...
/obj/effect/rune/manifest
	...
	cultist_desc = "manifesta um servo espiritual do Geômetro e permite que você ascenda como um espírito. O invocador não deve se mover de cima da runa, e sofrerá danos por cada espírito convocado."
...
/obj/effect/rune/manifest/can_invoke(mob/living/user)
	...
	if(user.has_status_effect(/datum/status_effect/cultghost))
		to_chat(user, span_cult_italic("Fantasmas não podem invocar mais fantasmas!"))
		fail_invoke()
...
/obj/effect/rune/manifest/invoke(list/invokers)
	...
	var/mob/living/user = invokers[1]
	var/turf/T = get_turf(src)
	var/choice = tgui_alert(user, "Você abre uma conexão com o reino espiritual...", "Spirit Realm", list("Summon a Cult Ghost", "Ascend as a Dark Spirit"))
	if(choice == "Summon a Cult Ghost")
		if(!is_station_level(T.z))
			to_chat(user, span_cult_italic("<b>O véu não é fraco o suficiente aqui para manifestar espíritos, você deve estar na estação!</b>"))
			return
		if(ghosts >= ghost_limit)
			to_chat(user, span_cult_italic("Você está sustentando muitos fantasmas para invocar mais!"))
			fail_invoke()
		...
		to_chat(new_human, span_cult_italic("<b>Você é um servo do Geômetro. Você foi feito semi-corpóreo pelo culto de Nar'Sie, e você deve servi-los a todo custo.</b>"))
		...
		while(!QDELETED(affecting))
			...
			if(affecting.health <= 10)
				to_chat(G, span_cult_italic("Seu corpo não pode mais manter a conexão!"))
				break
			sleep(0.5 SECONDS)
		CM.Remove(G)
		GM.Remove(G)
...
/obj/effect/rune/apocalypse
	...
	cultist_desc = "Um prenúncio do fim dos tempos. Cresce em força com o desespero do culto, mas sob o risco de... efeitos colaterais."
...
/obj/effect/rune/apocalypse/invoke(list/invokers)
	...
	visible_message(span_warning("Uma colossal onda de choque de energia explode da runa, desintegrando-a no processo!"))
