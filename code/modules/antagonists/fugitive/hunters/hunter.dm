...
/datum/antagonist/fugitive_hunter/greet()
	switch(backstory)
		if(HUNTER_PACK_COPS)
			to_chat(owner, span_bolddanger("A justiça chegou. Eu sou um membro da Spacepol!"))
			to_chat(owner, "<B>Os criminosos devem estar na estação, temos huds especiais implantados para reconhecê-los.</B>")
			to_chat(owner, "<B>Como perdemos todo o poder sobre essas megacorporações sem lei, é um mistério se a segurança deles cooperará conosco.</B>")
		if(HUNTER_PACK_RUSSIAN)
			to_chat(owner, span_danger("Ay blyat. Sou um contrabandista russo! Estávamos no meio do voo quando nossa carga foi transportada de nossa nave!"))
			to_chat(owner, span_danger("Fomos salvos por um homem de uniforme verde, prometendo o retorno seguro de nossos benefícios em troca de um favor:"))
			to_chat(owner, span_danger("Há uma estação local abrigando fugitivos que o homem está atrás, ele os quer de volta, mortos ou vivos."))
			to_chat(owner, span_danger("Não conseguiremos pagar sem nossa carga, então devemos fazer o que ele diz e capturá-los."))
		if(HUNTER_PACK_BOUNTY)
			to_chat(owner, span_danger("Hora de marcar. Sou um caçador de recompensas! Devemos chegar ao esconderijo do nosso alvo em breve."))
			to_chat(owner, span_danger("A reunião mencionou que nosso destino era uma estação de pesquisa. Um lugar incomum para um alvo tentar se esconder."))
			to_chat(owner, span_danger("Nosso cliente nos prometeu muito dinheiro, e pretendemos cumprir nossa entrega. Vamos esperar que seja um pagamento fácil..."))
		if(HUNTER_PACK_PSYKER)
			to_chat(owner, span_danger("Boa noite, somos Psyker Hunte... Não, Psyker Shikaris!"))
			to_chat(owner, span_danger("Um cérebro nos deu uma oferta que não podíamos recusar. Sequestramos alguns tolos por eles, e em troca temos um suprimento de GORE."))
			to_chat(owner, span_danger("Nosso suprimento de sangue está acabando. Como poderíamos dizer não? A farra deve continuar!"))
		if(HUNTER_PACK_MI13)
			to_chat(owner, span_danger("Agentes, detectamos um fugitivo procurado no espaço controlado por Nanotrasen."))
			to_chat(owner, span_danger("Sua missão é simples. Infiltrar a instalação e extrair o alvo, vivo ou morto."))
			to_chat(owner, span_danger("Esta é uma missão de infiltração em território inimigo hostil. Tenha cuidado, e evite ser pego se possível."))

	to_chat(owner, span_bolddanger("Você não é um antagonista em que você pode matar quem quiser, mas você pode fazer qualquer coisa para garantir a captura dos fugitivos, mesmo que isso signifique passar pela estação."))
