/datum/action/cooldown/spell/cosmic_rune
	name = "Cosmic Rune"
	desc = "Cria uma runa cósmica em sua posição, apenas dois podem existir de cada vez. Invocar uma runa o transporta para a outra.\
Qualquer um com uma marca estelar é transportado junto com você."
...
/obj/effect/cosmic_rune
	name = "cosmic rune"
	desc = "Uma runa estranha, que pode transportar pessoas para outro local."
	anchored = TRUE
...
/obj/effect/cosmic_rune/attack_hand(mob/living/user, list/modifiers)
	...
	if(.)
		return
	if(!linked_rune)
		balloon_alert(user, "Sem runa ligada!")
		fail_invoke()
		return
	if(!(user in get_turf(src)))
		balloon_alert(user, "Não perto o suficiente!")
		fail_invoke()
		return
	if(user.has_status_effect(/datum/status_effect/star_mark))
		balloon_alert(user, "Bloqueado pela marca estelar!")
		fail_invoke()
		return
	invoke(user)
...
/obj/effect/cosmic_rune/proc/fail_invoke()
	visible_message(span_warning("A runa pulsa com um pequeno flash de luz roxa, e então retorna ao normal."))
