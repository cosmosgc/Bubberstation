...
/datum/action/cooldown/spell/jaunt/space_crawl
	name = "Space Phase"
	desc = "Permite que você entre e saia da existência no espaço ou em uma área ao ar livre de baixa pressão."
...
/datum/action/cooldown/spell/jaunt/space_crawl/can_cast_spell(feedback = TRUE)
	...
	if(is_valid_turf())
		return TRUE
	if(feedback)
		to_chat(owner, span_warning("Você deve ficar no espaço, ou uma área ao ar livre com baixa pressão!"))
	return FALSE
...
/datum/action/cooldown/spell/jaunt/space_crawl/proc/do_spacecrawl(turf/our_turf, mob/living/jaunter)
	...
	if(!.)
		...
		to_chat(jaunter, span_warning("Você é incapaz de rastejar no espaço!"))
...
/datum/action/cooldown/spell/jaunt/space_crawl/proc/try_enter_jaunt(turf/our_turf, mob/living/jaunter)
	...
	our_turf.visible_message(span_warning("[jaunter] Afunda-se em [our_turf]!"))
...
/datum/action/cooldown/spell/jaunt/space_crawl/proc/try_exit_jaunt(turf/our_turf, mob/living/jaunter, force = FALSE)
	...
	our_turf.visible_message(span_boldwarning("[jaunter] Se levanta de [our_turf]!"))
	return TRUE
