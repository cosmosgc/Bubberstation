/obj/structure/lock_tear
	name = "???"
	desc = "Olha para trás. Não há razão para ficar. Corra."
...
/obj/structure/lock_tear/proc/poll_ghosts()
	var/list/candidates = SSpolling.poll_ghost_candidates("Would you like to be a random [span_notice("eldritch monster")] attacking the crew?", check_jobban = ROLE_SENTIENCE, role = ROLE_SENTIENCE, poll_time = 10 SECONDS, ignore_category = POLL_IGNORE_HERETIC_MONSTER, alert_pic = src, role_name_text = "Monstro Eldritch")
...
/obj/structure/lock_tear/proc/end_madness(datum/former_master)
	...
	visible_message(span_boldwarning("O rasgo no espasmo espacial desaparece!"))
...
/obj/structure/lock_tear/examine(mob/user)
	...
	if (!isobserver(user) || gathering_candidates)
		return
	. += span_notice("Você pode usar isso para entrar no mundo como um monstro imundo.")

/// Turn a ghost into an 'orrible beast
/obj/structure/lock_tear/proc/ghost_to_monster(mob/dead/observer/user, should_ask = TRUE)
	if(should_ask)
		var/ask = tgui_alert(user, "Tornar-se um monstro?", "Ascended Rift", list("Yes", "No"))
