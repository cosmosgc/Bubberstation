...
/datum/antagonist/nukeop/support/on_gain()
	...
	for(var/datum/mind/teammate_mind in nuke_team.members)
		...
		our_teammate.AddComponent( 			/datum/component/simple_bodycam, 			camera_name = "operative bodycam", 			c_tag = "[our_teammate.real_name]", 			network = OPERATIVE_CAMERA_NET, 			emp_proof = FALSE, 		)
...
/datum/antagonist/nukeop/support/proc/late_bodycam(datum/source, mob/living/new_teammate)
	SIGNAL_HANDLER
	new_teammate.AddComponent( 		/datum/component/simple_bodycam, 		camera_name = "operative bodycam", 		c_tag = "[new_teammate.real_name]", 		network = OPERATIVE_CAMERA_NET, 		emp_proof = FALSE, 	)
