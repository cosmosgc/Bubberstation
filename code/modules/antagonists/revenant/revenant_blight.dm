...
/datum/disease/revblight/stage_act(seconds_per_tick)
	...
	if(!finalstage)
		...
		if(SPT_PROB(1.5 * stage, seconds_per_tick))
			to_chat(affected_mob, span_revennotice("Você de repente sente[pick("sick and tired", "disoriented", "tired and confused", "nauseated", "faint", "dizzy")]..."))
	...
	switch(stage)
		...
		if(5)
			if(!finalstage)
				finalstage = TRUE
				to_chat(affected_mob, span_revenbignotice("Você sente como[pick("nothing's worth it anymore", "nobody ever needed your help", "nothing you did mattered", "everything you tried to do was worthless")]."))
