...
/datum/component/spy_uplink/proc/on_examine(obj/item/source, mob/user, list/examine_list)
	...
	examine_list += span_notice("- [EXAMINE_HINT("Use it in hand")]para ver sua lista de recompensas.")
	examine_list += span_notice("- [EXAMINE_HINT("Right click")]com ele em um alvo de recompensa para reivindicá-lo.")
...
/datum/component/spy_uplink/proc/steal_process(atom/movable/stealing, mob/living/spy, datum/spy_bounty/bounty)
	...
	to_chat(spy, span_notice("Recompensa completa! Você foi recompensado com\a [reward].		[reward.loc == spy ? "" : " <i>Find it at your feet.</i>"]"))
