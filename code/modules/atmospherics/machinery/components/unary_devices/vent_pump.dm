...
/obj/machinery/atmospherics/components/unary/vent_pump/welder_act(mob/living/user, obj/item/welder)
	...
	if(welder.use_tool(src, user, 20, volume=50))
		if(!welded)
			user.visible_message(span_notice("[user] welds the vent shut."), span_notice("Você soldou a ventilação fechada."), span_hear("Você ouve solda."))
			welded = TRUE
		else
			user.visible_message(span_notice("[user] unwelded the vent."), span_notice("Você desgastou a ventilação."), span_hear("Você ouve solda."))
			welded = FALSE
		update_appearance(UPDATE_ICON)
...
/obj/machinery/atmospherics/components/unary/vent_pump/attack_alien(mob/user, list/modifiers)
	if(!welded || !(do_after(user, 2 SECONDS, target = src)))
		return
	user.visible_message(span_warning("[user] furiously claws at [src]!"), span_notice("Você consegue limpar o material bloqueando a ventilação."), span_hear("Você ouve barulhos altos."))
	welded = FALSE
	update_appearance(UPDATE_ICON)
	pipe_vision_img = image(src, loc, dir = dir)
	SET_PLANE_EXPLICIT(pipe_vision_img, ABOVE_HUD_PLANE, src)
