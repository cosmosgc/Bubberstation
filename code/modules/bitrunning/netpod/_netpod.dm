...
/obj/machinery/netpod/examine(mob/user)
	...
	if(isnull(occupant))
		if(panel_open)
			...
			. += span_notice("Sua escotilha pode ser[EXAMINE_HINT("pried")] [state_open ? "closed" : "open"].")
	...
	if(!isobserver(user))
		...
		. += span_infoplain("Alt-click para[copy_body ? "disable" : "enable"]Escaneamento de ocupantes.")
