...
/datum/spacevine_mutation/cold_proof
	name = "Cold proof"
	description = "Proporcionar imunidade a danos frios."
	hue = "#0BD5D9"
	quality = MINOR_NEGATIVE
	severity = SEVERITY_AVERAGE
...
/datum/spacevine_mutation/aggressive_spread/aggrospread_act(obj/structure/spacevine/vine, mob/living/living_mob)
	...
	if(!HAS_TRAIT(victim, TRAIT_PIERCEIMMUNE))
		...
		if(thorn && prob(40)) //If we found the thorns mutation there is now a chance to get stung instead of lashed or smashed.
			...
			span_userdanger("Você está pregado por um espinho afiado!"))
		...
		if(prob(80))
			...
			span_userdanger("Você é dilacerado por uma explosão de videiras!"))
	...
	span_userdanger("Você está esmagado por uma grande videira!"))
...
/datum/spacevine_mutation/timid
	name = "Timid"
	description = "Esconda-se como vinhas sob estruturas e como impedir de se misturar."
	hue = "#a4a9ac"
	quality = POSITIVE
	severity = SEVERITY_MINOR
