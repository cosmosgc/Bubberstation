...
/obj/machinery/fishing_portal_generator/proc/link_fishing_spot(datum/fish_source/source, atom/spot, mob/living/user)
	...
	for(var/other_spot in linked_fishing_spots)
		...
		if(stored == source)
			spot.balloon_alert(user, "Já conectado!")
...
/obj/machinery/fishing_portal_generator/examine(mob/user)
	. = ..()
	. += span_notice("Você pode desbloquear novas configurações do portal completando experimentos de varredura de peixes,\
Ou conectando-o a outros pontos de pesca com uma multitool.")
