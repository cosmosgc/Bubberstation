...
/datum/interview/proc/deny(client/denied_by)
	...
	if (owner)
		SEND_SOUND(owner, sound('sound/effects/adminhelp.ogg'))
		to_chat(owner, "<font color='red' size='4'><b>-Redação de entrevista.</b></font>" \
			+ "\n<span class='adminsay'>Infelizmente sua entrevista foi negada. Por favor, tente enviar outro questionário." \
			+ "Pode fazer isso em três minutos.</span>", confidential = TRUE)
...
/mob/dead/new_player/proc/open_interview()
	...
	if (M?.client?.interviewee)
		...
		if (I) // we can be returned nothing if the user is on cooldown
			...
			to_chat(usr, "<span class='adminsay'>Você está em repouso para entrevistas. Por favor." \
				+ "Espere pelo menos 3 minutos antes de começar um novo questionário.</span>", confidential = TRUE)
