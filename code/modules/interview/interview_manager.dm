...
/datum/interview_manager/proc/enqueue(datum/interview/to_queue)
	...
	if (admins_present <= 0 && to_queue.owner)
		to_chat(to_queue.owner, span_notice("Nenhum administrador ativo está online, sua entrevista foi enviada através da TGS para administradores que estão disponíveis. Isso pode usar IRC ou Discórdia."))
	for(var/client/X in GLOB.admins)
		if(X.prefs.toggles & SOUND_ADMINHELP)
			SEND_SOUND(X, sound('sound/effects/adminhelp.ogg'))
		window_flash(X, ignorepref = TRUE)
		to_chat(X, span_adminhelp("[to_queue.link_self()]Para[ckey]em fila para revisão. Posição atual na fila:[to_queue.pos_in_queue]"), confidential = TRUE)
