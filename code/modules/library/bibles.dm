...
/obj/item/book/bible/Initialize(mapload)
	...
	AddComponent(		/datum/component/bullet_intercepting,		active_slots = ITEM_SLOT_SUITSTORE,		on_intercepted = CALLBACK(src, PROC_REF(on_intercepted_bullet)),		block_charges = 1,	)
...
/obj/item/book/bible/proc/curse_heathen(datum/source, mob/living/user, obj/item/burning_tool)
	...
	if(existing_omen)
		to_chat(user, span_userdanger("[deity_name] <b>SMITE</b>Você!"))
...
/obj/item/book/bible/interact_with_atom(atom/bible_smacked, mob/living/user, list/modifiers)
	...
	if(istype(bible_smacked, /obj/item/melee/cultblade/haunted) && !IS_CULTIST(user))
		...
		if(!sword_smacked.bound)
			sword_smacked.balloon_alert(user, "Deve ser amarrado!")
...
/obj/item/book/bible/syndicate/Initialize(mapload)
	...
	AddComponent(/datum/component/effect_remover, 		success_feedback = "You disrupt the magic of %THEEFFECT with %THEWEAPON.", 		success_forcesay = "BEGONE FOUL MAGIKS!!", 		tip_text = "Clear rune", 		effects_we_clear = list(/obj/effect/rune, /obj/effect/heretic_rune, /obj/effect/cosmic_rune), 	)
