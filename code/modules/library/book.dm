...
#define ASSERT_DATA(data_list, data_field) 	if(!data_list[data_field]) { stack_trace("[type] - [data_list["id"]] lacks [data_field] information!"); }
...
/obj/item/tgui_book/manual/dsm
	...
	desc = "O Manual de Diagnóstico Espacial e Estatística de Transtornos Mentais, um livro abrangente sobre todos os transtornos mentais conhecidos. Na sua 35a edição, embora seja para uma atualização..."
	icon_state = "book8"
	ui_name = "DSMBook"
...
/obj/item/tgui_book/manual/idc
	...
	desc = "A Classificação Interplanetária de Doenças, um livro abrangente sobre doenças e doenças conhecidas. Em sua 27a edição, embora seja para uma atualização..."
	icon_state = "book7"
	ui_name = "IDCBook"
...
/obj/item/tgui_book/manual/idc/ui_static_data(mob/user)
	...
	if(!disease_info)
		...
		var/list/advanced_virus_info = list(
			...
			"desc" = "A catch-all category for diseases that are too complex to document in their entirety. 				The Virology department is known for bio-engineering these diseases, as such their danger can vary wildly. 				These diseases are often a combination of various symptoms, each cataloged in this book individually.",
			"illness" = "N/A",
