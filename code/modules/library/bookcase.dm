...
/obj/structure/bookcase/examine(mob/user)
	...
	switch(state)
		if(BOOKCASE_UNANCHORED)
			. += span_notice("Tem...<b>Pequena rachadura</b>visível no painel de trás.")
		if(BOOKCASE_ANCHORED)
			. += span_notice("Há espaço dentro para um<i>madeira</i>Prateleira.")
		if(BOOKCASE_FINISHED)
			. += span_notice("Tem...<b>Pequena rachadura</b>visível na prateleira.")
...
/obj/structure/bookcase/attackby(obj/item/attacking_item, mob/user, list/modifiers, list/attack_modifiers)
	...
	if(state == BOOKCASE_ANCHORED)
		...
		if(attacking_item.tool_behaviour == TOOL_WRENCH)
			...
			balloon_alert(user, "Desenrosque uma moldura.")
			set_anchored(FALSE)
			return
		return ..()
