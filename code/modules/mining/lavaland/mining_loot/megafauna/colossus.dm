...
/datum/action/item_action/organ_action/colossus/IsAvailable(feedback = FALSE)
	...
	if(world.time < cords.next_command)
		if (feedback)
			owner.balloon_alert(owner, "Espere.[DisplayTimeText(cords.next_command - world.time)]!")
		return FALSE
	if(isliving(owner))
...
/obj/machinery/anomalous_crystal/theme_warp/Initialize(mapload)
	...
	observer_desc = "Este cristal muda a área ao redor para combinar com o tema\"[terrain_theme.name]\"."
