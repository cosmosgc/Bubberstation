...
/mob/living/basic/gorilla/Initialize(mapload)
	...
	AddComponent(
		/datum/component/amputating_limbs, 		surgery_time = 0 SECONDS, 		surgery_verb = "punches",	)
...
/mob/living/basic/gorilla/examine(mob/user)
	...
	if (!HAS_MIND_TRAIT(user, TRAIT_EXAMINE_FITNESS))
		return
	. += span_notice("Este animal parece estar em alta condição física e ainda assim provavelmente nunca trabalhou um dia em sua vida. O potencial inexplorado é quase assustador.")
...
/mob/living/basic/gorilla/genetics
	...
	maxHealth = 180
	health = 180
	desc = "Um gorila criou via\"ciência genética avançada\"Apesar de não ser tão forte quanto seus irmãos selvagens, este símio ainda dá um soco."
