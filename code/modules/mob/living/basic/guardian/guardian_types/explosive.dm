...
/datum/action/cooldown/mob_cooldown/explosive_booby_trap/Activate(atom/target)
	...
	target.AddComponent(		/datum/component/direct_explosive_trap, 		saboteur = owner, 		expire_time = decay_time, 		glow_colour = glow_colour,		explosive_checks = CALLBACK(src, PROC_REF(validate_target)), 		triggering_signals = boom_signals, 	)
