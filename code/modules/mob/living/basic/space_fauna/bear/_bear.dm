...
/mob/living/basic/bear/butter/Initialize(mapload)
	...
	AddComponent(/datum/component/regenerator,		regeneration_delay = 1 SECONDS,		brute_per_second = 5,		outline_colour = COLOR_YELLOW,	)
...
/mob/living/basic/bear/butter/on_craft_completion(list/components, datum/crafting_recipe/current_recipe, atom/crafter) //Borrowed code from Cak, allows the brain used to actually control the bear.
	...
	to_chat(src, "[span_boldbig("You are a butter bear!")]<b> You're a mostly harmless bear/butter hybrid that everyone loves. People can take bites out of you if they're hungry, but you regenerate health 	so quickly that it generally doesn't matter. You're remarkably resilient to any damage besides this and it's hard for you to really die at all. You should go around and bring happiness and 	free butter to the station!</b>")
