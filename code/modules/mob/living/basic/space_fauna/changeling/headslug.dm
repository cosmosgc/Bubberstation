...
/mob/living/basic/headslug
	name = "headslug"
	desc = "Uma criatura pequena, tipo lesma, com uma grande e escancarada mãe. Está coberto de um muco grosso e viscoso."
	icon_state = "headslug"
...
/mob/living/basic/headslug/examine(mob/user)
	...
	if(stat != DEAD)
		if(isnull(client))
			. += span_notice("Parece estar se movendo de forma apático.")
		else
			. += span_warning("Está se movendo inteligentemente!")
	if (egg_lain)
		. += span_notice("Seu equipamento reprodutivo parece ter murchado.")
...
/mob/living/basic/headslug/proc/check_and_implant(mob/living/basic/attacker, atom/target)
	...
	if(HAS_TRAIT(victim, TRAIT_XENO_HOST))
		target.balloon_alert(src, "Já está grávida!") // Maybe the worst balloon alert in the codebase
		return

	if(!infect(victim))
		target.balloon_alert(src, "Não conseguiu implantar o óvulo!")
		stack_trace("[key] in [src] failed to implant egg in [victim], despite all checks suggesting it should have worked!")
		return

	egg_lain = TRUE
	to_chat(src, span_userdanger("Com nosso ovo posto, nossa morte se aproxima rapidamente..."))
