...
/mob/living/carbon/alien/adult/attack_hulk(mob/living/carbon/human/user)
	...
	visible_message(span_danger("[user] [hitverb]s [src]!"), \
					span_userdanger("[user] [hitverb]s you!"), span_hear("Você ouve um som doentio de carne batendo em carne!"), COMBAT_MESSAGE_RANGE, user)
...
/mob/living/carbon/alien/adult/attack_hand(mob/living/carbon/human/user, list/modifiers)
	...
	if (prob(90))
		...
		visible_message(span_danger("[user] punches [src]!"), \
						span_userdanger("[user] punches you!"), span_hear("Você ouve um som doentio de carne batendo em carne!"), COMBAT_MESSAGE_RANGE, user)
		...
		if ((stat != DEAD) && (damage > 9 || prob(5)))//Regular humans have a very small chance of knocking an alien down.
			...
			visible_message(span_danger("[user] knocks [src] down!"), \
							span_userdanger("[user] knocks you down!"), span_hear("Você ouve um som doentio de carne batendo em carne!"), null, user)
		...
		visible_message(span_danger("[user]'s punch misses [src]!"), \
						span_danger("You avoid [user]'s punch!"), span_hear("Você ouve um shoosh!"), COMBAT_MESSAGE_RANGE, user)
