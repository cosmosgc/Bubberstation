...
/datum/species/golem/proc/on_examine(mob/living/carbon/human/source, mob/living/examiner, list/examine_text)
	...
	examine_text += span_warning("Este golem parece estar em um estado de desânimo.\
Pode ser possível reconstruí-lo adicionando minerais em sua forma.")
	...
	if(source.nutrition < NUTRITION_LEVEL_STARVING)
		if(!early_warning && COOLDOWN_FINISHED(src, warning_cd) && source.stat < UNCONSCIOUS)
			source.visible_message(
				...
				span_bolddanger("Você sente sua forma se desestabilizando enquanto falta material para se sustentar!\
Encontre alguns minerais para comer em breve, ou você pode desmoronar!"),
				visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
			)
			early_warning = TRUE
	...
	if(source.nutrition < 50)
		if(!final_warning && COOLDOWN_FINISHED(src, warning_cd) && source.stat < UNCONSCIOUS)
			source.visible_message(
				...
				span_userdanger("Sua forma estremece violentamente enquanto você quase completa desestabilização!\
Coma alguns minerais rapidamente, ou você pode desmoronar!"),
				visible_message_flags = ALWAYS_SHOW_SELF_MESSAGE,
			)
			final_warning = TRUE
			COOLDOWN_START(src, warning_cd, 1 MINUTES)
...
/datum/species/golem/create_pref_unique_perks()
	...
	to_add += list(list(
		...
		SPECIES_PERK_DESC = "Litóides são criaturas feitas de minerais em vez de\
Sangue e carne. Eles são fortes e imunes a muitos perigos ambientais e pessoais.\
como fogo, radiação, falta de ar, lava, vírus e desmembramento.",
	))
	...
	to_add += list(list(
		...
		SPECIES_PERK_DESC = "Os Golems podem ver pouco nas trevas, minerais de sentido, pedras quebradas, e minha pedra com suas próprias mãos.\
Eles podem até mesmo cheirar minérios em uma fornalha interna, se seu ambiente ao redor é quente o suficiente.",
	))
	...
	to_add += list(list(
		...
		SPECIES_PERK_DESC = "Manter a pedra animadora da força é difícil. Litóides devem comer com frequência.\
para evitar o retorno a estátuas inanimadas, e apenas derivar nutrição de comer minerais.",
	))
