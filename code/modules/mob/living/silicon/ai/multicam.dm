...
/mob/living/silicon/ai/proc/drop_new_multicam(silent = FALSE)
	if(!CONFIG_GET(flag/allow_ai_multicam))
		if(!silent)
			to_chat(src, span_warning("Esta ação está atualmente desativada. Contate um administrador para ativar esse recurso."))
		return
	if(!eyeobj)
		return
	...
	if(!silent)
		to_chat(src, span_notice("Adicionada nova janela multicâmera."))
	return C
...
/mob/living/silicon/ai/proc/start_multicam()
	...
	if(!GLOB.ai_camera_room_landmark)
		to_chat(src, span_warning("Esta função não está disponível neste momento."))
		return
	multicam_on = TRUE
	refresh_multicam()
	to_chat(src, span_notice("Modo de visualização de múltiplas câmeras ativado."))
...
/mob/living/silicon/ai/proc/end_multicam()
	...
	if(client)
		for(var/V in multicam_screens)
			...
			P.unshow_to(client)
	reset_perspective()
	to_chat(src, span_notice("Modo de visualização de múltiplas câmeras desativado."))
