/mob/living/silicon/proc/show_laws()
	laws_sanity_check()
	var/list/law_box = list(span_bold("Obedeçam a essas leis:"))
...
/mob/living/silicon/proc/deadchat_lawchange()
	...
	deadchat_broadcast("'s<b>Como Leis foram lamadas.</b> <a href='byond://?src=[REF(src)]&dead=1&printlawtext=[url_encode(lawtext)]'>Vista</a>", span_name("[src]"), follow_target=src, message_type=DEADCHAT_LAWCHANGE)

/mob/living/silicon/proc/post_lawchange(announce = TRUE)
	throw_alert(ALERT_NEW_LAW, /atom/movable/screen/alert/newlaw)
	if(announce && last_lawchange_announce != world.time)
		to_chat(src, span_bolddanger("Suas leis foram lamadas."))
