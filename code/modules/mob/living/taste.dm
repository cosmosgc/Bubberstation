...
/mob/living/proc/check_tasting_blocks()
	...
	if(get_timed_status_effect_duration(/datum/status_effect/hallucination) > 100 SECONDS && prob(25))
		var/text_output = pick("spiders","dreams","nightmares","the future","the past","victory",			"defeat","pain","bliss","revenge","poison","time","space","death","life","truth","lies","justice","memory",			"regrets","your soul","suffering","music","noise","blood","hunger","the american way")
		send_taste_message(text_output)
		return TRUE
...
/mob/living/proc/send_taste_message(tastes_text)
	...
	to_chat(src, span_notice("Você pode provar.[tastes_text]."))
