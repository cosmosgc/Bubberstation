...
/client/proc/Process_Grab()
	...
	if(HAS_TRAIT(mob, TRAIT_INCAPACITATED))
		...
		COOLDOWN_START(src, move_delay, 1 SECONDS)
		to_chat(src, span_warning("Você está preso! Não pode se mexer!"))
		return TRUE
		...
		if(INCORPOREAL_MOVE_JAUNT) //Incorporeal move, but blocked by holy-watered tiles and salt piles.
			...
			if(stepTurf)
				...
				if(stepTurf.turf_flags & NOJAUNT)
					to_chat(L, span_warning("Alguma aura estranha está bloqueando o caminho."))
					return
				if(locate(/obj/effect/blessing) in stepTurf)
					to_chat(L, span_warning("As energias sagradas bloqueiam seu caminho!"))
					return
...
/mob/living/proc/toggle_move_intent()
	if(move_intent == MOVE_INTENT_RUN)
		...
		if (HAS_TRAIT(src,TRAIT_NORUNNING))
			to_chat(src, "Você não consegue fugir.")
			return FALSE
