...
/obj/effect/mob_spawn/ghost_role/human/golem/proc/give_directive(mob/living/new_spawn)
	...
	if(is_station_level(new_spawn.z))
		return
	to_chat(new_spawn, "Construa conchas de golem no autolate, e dê folhas minerais refinadas para as conchas para trazê-las à vida! Você geralmente é um grupo pacífico a menos que provocado.")
