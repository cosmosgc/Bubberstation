...
/obj/item/mod/construction/lavalandcore
	...
	desc = "Uma estranha flor dos desolados resíduos de lavalândia. Ele pulsa com um brilho roxo brilhante. Sua forma é notavelmente semelhante à de um núcleo MOD."
...
/obj/item/mod/construction/lavalandcore/examine(mob/user)
	. = ..()
	. += span_notice("Você provavelmente poderia anexar alguns<b>Fios.</b>Um ele...")
...
/obj/item/mod/construction/shell/examine(mob/user)
	...
	switch(step)
		...
		if(BOOTS_STEP)
			display_text = "A montagem parece<b>Não está seguro.</b>..."
		if(WRENCHED_ASSEMBLY_STEP)
			display_text = "A montagem parece<b>Soltar</b>..."
		if(SCREWED_ASSEMBLY_STEP)
			display_text = "Tudo o que falta é<b>Revistamento externo</b>..."
		...
		if(GAUNTLETS_STEP)
			if(istype(part, /obj/item/mod/construction/boots)) //Construct
				...
				if(part.use_tool(src, user, 0, volume=30))
					...
					balloon_alert(user, "Como luvas foram removidas.")
					gauntlets = null
					step = CHESTPLATE_STEP
		if(BOOTS_STEP)
