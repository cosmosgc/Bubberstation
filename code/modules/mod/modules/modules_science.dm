...
/obj/item/mod/module/reagent_scanner
	...
	desc = "Um módulo baseado em HUDs de Nanotrasen orientados à pesquisa, que é capaz de digitalizar o conteúdo de\
Contêineres e projetando as informações em um formato fácil de ler na tela do usuário.\
Não pode detectar sabores, então é com você."
	icon_state = "scanner"
...
/obj/item/mod/module/reagent_scanner/advanced
	...
	desc = "Um módulo avançado com todas as características dos HUDs Nanotrasen orientados para pesquisa, isso é capaz de escanear\
o conteúdo dos contêineres e projetando as informações em um formato fácil de ler na tela do usuário.\
Também contém um scanner de pesquisa e um sensor de explosão que dá detalhes sobre explosões próximas.\
Nenhuma melhoria foi feita para permitir que ele detecte sabores, então isso ainda depende de você."
	complexity = 0
	removable = FALSE
...
/obj/item/mod/module/anomaly_locked/teleporter/on_select_use(atom/target)
	...
	if(target_turf.is_blocked_turf_ignore_climbable() || !los_check(mod.wearer, target, pass_args = PASSTABLE|PASSGLASS|PASSGRILLE|PASSMOB|PASSMACHINE|PASSSTRUCTURE|PASSFLAPS|PASSWINDOW))
		balloon_alert(mod.wearer, "Destino bloqueado!")
		return
