...
/obj/item/mod/module/eradication_lock
	...
	desc = "Um módulo que lembra o dono original do terno, até mesmo versões alternativas do universo. Quando um não proprietário entrar, o bloqueio de erradicação começará a erradicar o processo da linha do tempo... com o intruso dentro. Não do jeito que quer, então é um bom dissuasor."
...
/obj/item/mod/module/eradication_lock/on_use(mob/activator)
	...
	balloon_alert(activator, "usuário lembrou")
...
/obj/item/mod/module/eradication_lock/proc/on_mod_activation(datum/source, mob/user)
	...
	if(!true_owner_ckey || user.ckey == true_owner_ckey)
		return NONE
	if(user == mod.wearer)
		to_chat(mod.wearer, span_userdanger("\"MODsuit comprometido por habitante da linha do tempo! Erradicando...\""))
...
/obj/item/mod/module/eradication_lock/proc/on_mod_removal(datum/source, mob/user)
	...
	if(true_owner_ckey && user.ckey != true_owner_ckey)
		to_chat(mod.wearer, span_userdanger("\"Fusão de tempo detectada! Erradicando...\""))
		new /obj/structure/chrono_field(user.loc, user)
...
/obj/item/mod/module/rewinder
	...
	desc = "Um módulo que pode puxar o usuário de volta através do tempo dado um ponto de âncora para puxar. Uma ferramenta muito útil para fazer o trabalho, mas tenha em mente as fechaduras do terno por razões de segurança enquanto prepara um rebobinamento."
	icon_state = "rewinder"
	module_type = MODULE_USABLE
	removable = FALSE
...
/obj/item/mod/module/rewinder/on_use(mob/activator)
	balloon_alert(activator, "Ponto de ancoragem definido.")
...
/obj/item/mod/module/rewinder/proc/on_activate_block(datum/source, user)
	SIGNAL_HANDLER
	balloon_alert(user, "Não enquanto rebobinar!")
	return MOD_CANCEL_ACTIVATE
...
/obj/item/mod/module/timestopper
	...
	desc = "Um módulo que pode parar o tempo em um pequeno raio ao redor do usuário... pelo tempo que quiserem! Ótimo para monólogos ou pausas para almoço. Tenha em mente que mover vai acabar com a parada, e o módulo tem um período de resfriamento pesado para evitar erros de realidade."
	icon_state = "timestop"
	module_type = MODULE_USABLE
	removable = FALSE
	use_energy_cost = DEFAULT_CHARGE_DRAIN * 5
...
/obj/item/mod/module/timestopper/used(mob/activator)
	if(timestop)
		mod.balloon_alert(activator, "Já está congelando!")
		return FALSE
	return ..()
...
/obj/item/mod/module/timestopper/proc/on_module_triggered(datum/source)
	SIGNAL_HANDLER
	balloon_alert(mod.wearer, "Não enquanto para o tempo!")
	return MOD_ABORT_USE
...
/obj/item/mod/module/timestopper/proc/on_activate_block(datum/source, user)
	SIGNAL_HANDLER
	balloon_alert(user, "Não enquanto para o tempo!")
	return MOD_CANCEL_ACTIVATE
...
/obj/item/mod/module/timeline_jumper
	...
	desc = "Um módulo usado para atravessar linhas do tempo, fazendo o usuário entrar e sair do fluxo de eventos."
...
/obj/item/mod/module/timeline_jumper/used(mob/activator)
	...
	if(noteleport_check && !check_teleport_valid(mod.wearer, get_turf(mod.wearer)))
		to_chat(activator, span_danger("Alguma força chata e universal está entre você e o[phased_mob ? "current timeline" : "stream between timelines"]."))
		return FALSE
	return ..()
...
/obj/item/mod/module/timeline_jumper/proc/on_activate_block(datum/source, user)
	...
	to_chat(user, span_warning("Desativar seu terno enquanto entre linhas do tempo seria uma péssima ideia."))
	return MOD_CANCEL_ACTIVATE
...
/obj/item/mod/module/tem
	...
	desc = "O dispositivo de correção de um quarto grupo dimensional fora do tempo usado para mudar o destino de uma linha do tempo. Este dispositivo é capaz de limpar um ser do fluxo temporal. Nunca são, nunca foram, nunca serão."
...
/obj/item/mod/module/tem/proc/field_connect(obj/structure/chrono_field/field)
	if(field.tem)
		if(field.captured)
			balloon_alert(mod.wearer, "Já tem conexão!")
		field_disconnect(field)
		return
	...
	field.tem = src
	if(field.captured)
		balloon_alert(mod.wearer, "Conexão estabilizada")
...
/obj/item/mod/module/tem/proc/field_disconnect(obj/structure/chrono_field/field)
	if(field)
		if(field.tem == src)
			field.tem = null
		if(field.captured)
			balloon_alert(mod.wearer, "Ligação perdida!")
	field = null
	startpos = null
...
/obj/structure/chrono_field
	...
	desc = "Uma aura de energia do espaço-tempo."
...
/obj/structure/chrono_field/process(seconds_per_tick)
	...
	if(timetokill > initial(timetokill))
		...
		to_chat(captured, span_notice("Como a última essência de seu ser é apagada do tempo, você é levado de volta para sua memória mais agradável. Você se sente feliz..."))
