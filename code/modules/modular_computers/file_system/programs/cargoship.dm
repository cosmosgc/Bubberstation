/datum/computer_file/program/shipping
	...
	extended_desc = "Um aplicativo de combinação de impressoras que permite que computadores modulares imprimam códigos de barras para fácil digitalização e transporte."
	size = 6
...
/datum/computer_file/program/shipping/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	switch(action)
		...
		if("resetid")
			payments_acc = null
		if("setsplit")
			var/potential_cut = input("Quanto você gostaria de pagar pelo cartão registrado?","Percentage Profit ([round(cut_min*100)]% - [round(cut_max*100)]%)") as num|null
		...
		if("print")
			if(computer.stored_paper <= 0)
				...
				return TRUE
			if(!payments_acc)
				to_chat(usr, span_notice("Erro de software: por favor, defina um usuário atual primeiro."))
				return TRUE
			...
			to_chat(usr, span_notice("O computador imprime um código de barras."))
