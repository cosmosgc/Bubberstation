...
/obj/item/camera/siliconcam/proc/toggle_camera_mode(mob/user, sound = TRUE)
	if(user.click_intercept == src)
		...
		if(sound)
			balloon_alert(user, "Não consigo ativar o modo de câmera!")
...
/obj/item/camera/siliconcam/proc/selectpicture(mob/user)
	...
	if(!length(stored))
		user.balloon_alert(user, "Sem fotos armazenadas!")
		return
...
/obj/item/camera/siliconcam/robot_camera/after_picture(mob/living/silicon/robot/user, datum/picture/picture)
	if(istype(user) && istype(user.connected_ai))
		...
		balloon_alert(user, "Imagem gravada e carregada")
	else
		var/number = stored.len
		picture.picture_name = "Image [number] (taken by [loc.name])"
		stored[picture] = TRUE
		balloon_alert(user, "imagem gravada e salva localmente")
...
/obj/item/camera/siliconcam/robot_camera/proc/borgprint(mob/living/silicon/robot/user)
	if(!istype(user) || user.toner < printcost)
		balloon_alert(user, "Não chega de toner!")
		return
	var/datum/picture/selection = selectpicture(user)
	if(!istype(selection))
	...
	user.visible_message(span_notice("[user.name] spits out a photograph from a narrow slot on its chassis."), span_notice("Você imprime uma fotografia."))
	balloon_alert(user, "Fotografia impressa")
