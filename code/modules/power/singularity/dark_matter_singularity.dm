...
/obj/singularity/dark_matter
	...
	desc = "<i>\"É bonito e horrível,\
um paradoxo cósmico que desafia toda a lógica. Não posso.\
Tire meus olhos dele, mesmo sabendo que poderia.\
devorar todos nós em um instante.\
		\"</i><br>- Engenheiro Chefe Tenshin Nakamura"
...
/obj/singularity/dark_matter/examine(mob/user)
	...
	if(consumed_supermatter)
		...
		. += span_warning("<i>\"O aspecto mais perturbador da singularidade é seu\
aparente atração por organismos vivos. Parece sentir\
Sua presença e se movem em direção a eles a uma velocidade surpreendentemente rápida.\
Observamos que consome vários espécimes de flora e fauna que\
Nós coletamos deste setor. A singularidade não parece\
para cuidar de outros objetos inanimados ou máquinas, mas vai consumir\
São todos iguais. Tentamos nos comunicar com ele usando vários\
métodos, mas não recebeu resposta.\"</i><br>- Diretor de Pesquisa Huey Knorr")
