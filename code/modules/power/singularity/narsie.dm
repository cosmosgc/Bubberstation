...
/obj/narsie/proc/start_ending_the_round()
	...
	singularity = WEAKREF(AddComponent(
		/datum/component/singularity, 		bsa_targetable = FALSE, 		consume_callback = CALLBACK(src, PROC_REF(consume)), 		consume_range = NARSIE_CONSUME_RANGE, 		disregard_failed_movements = TRUE, 		grav_pull = NARSIE_GRAV_PULL, 		roaming = FALSE, /* This is set once the animation finishes */ 		singularity_size = NARSIE_SINGULARITY_SIZE, 	))
...
/proc/narsie_end_begin_check()
	...
	priority_announce(
		...
		title = "[command_name()]Assuntos Dimensionais Superiores",
