...
/obj/machinery/power/smes/examine(user)
	...
	. += span_notice("Seu painel de manutenção pode ser[EXAMINE_HINT("screwed")] [panel_open ? "closed" : "opened"].")
	if(panel_open)
		if(!terminal)
			. += span_notice("Pode ser.[EXAMINE_HINT("pried")]Separados.")
		. += span_notice("Pode.[EXAMINE_HINT("wrenched")]Para girar.")

	if(!terminal)
		. += span_warning("Um terminal que requer[EXAMINE_HINT("10 cable pieces")]Precisa ser instalado!")
	else if(panel_open)
		. += span_notice("O terminal pode ser[EXAMINE_HINT("cut")]Separados.")
