	...
	if(!targeted_gun.pin)
		...
		if(gun_insert(user, targeted_gun))
			if(old_pin)
				...
				balloon_alert(user, "Pino de dispersão inserido")
	else
	...
	switch(license_request)
		if("Yes")
			if(multi_payment)
				...
				to_chat(user, span_notice("Contratos de aluguel de armas concordaram, tem um dia seguro!"))
