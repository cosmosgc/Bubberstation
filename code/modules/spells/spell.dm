...
/datum/action/cooldown/spell/proc/can_cast_spell(feedback = TRUE)
	...
	if((spell_requirements & SPELL_REQUIRES_MIME_VOW) && !HAS_MIND_TRAIT(owner, TRAIT_MIMING))
		...
		if(feedback)
			to_chat(owner, span_warning("Você deve se dedicar ao silêncio primeiro!"))
		return FALSE
	...
	if(ishuman(owner))
		if(spell_requirements & SPELL_REQUIRES_WIZARD_GARB)
			...
			if(!(human_owner.wear_suit?.clothing_flags & CASTING_CLOTHES) && !ismonkey(human_owner)) // Monkeys don't need robes to cast as they are inherently imbued with power from the banana dimension
				if(feedback)
					to_chat(owner, span_warning("Você não se sente forte o suficiente sem seu roupão!"))
				return FALSE
			if(!(astype(human_owner.head, /obj/item/clothing)?.clothing_flags & CASTING_CLOTHES) && !(human_owner.glasses?.clothing_flags & CASTING_CLOTHES))
				if(feedback)
					to_chat(owner, span_warning("Você não se sente forte o suficiente sem seu chapéu!"))
				return FALSE
		...
		switch(invocation_type)
			if(INVOCATION_WHISPER, INVOCATION_SHOUT)
				if(!caster.get_organ_slot(ORGAN_SLOT_TONGUE))
					invocation(caster)
					to_chat(caster, span_warning("Sua falta de língua está dificultando dizer as palavras corretas.[src]..."))
