/datum/action/cooldown/spell/aoe/sacred_flame
	name = "Sacred Flame"
	desc = "Torna todos à sua volta mais inflamáveis, e se acende em chamas."
...
/datum/action/cooldown/spell/aoe/sacred_flame/cast_on_thing_in_aoe(mob/living/victim, mob/living/caster)
	...
	if(victim != caster)
		to_chat(victim, span_warning("Você de repente se sente muito inflamável."))

/datum/action/cooldown/spell/aoe/sacred_flame/cast(mob/living/cast_on)
	. = ..()
	cast_on.ignite_mob()
	to_chat(cast_on, span_danger("Você sente uma chama que ruge dentro de você!"))
