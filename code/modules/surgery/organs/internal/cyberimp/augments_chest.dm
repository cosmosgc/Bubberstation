...
/obj/item/organ/cyberimp/chest/reviver/proc/try_heal()
	if(reviving)
		if(owner.stat == CONSCIOUS)
			...
			reviving = FALSE
			to_chat(owner, span_notice("Seu implante reanimador desliga e começa a recarregar. Estará pronto novamente.[DisplayTimeText(revive_cost)]."))
		else
...
/obj/item/organ/cyberimp/chest/thrusters
	...
	desc = "Um conjunto implantável de portas propulsoras. Eles usam o gás do ambiente ou dos internos do sujeito para propulsão em áreas de gravidade zero. Ao contrário de jetpacks normais, este dispositivo não tem sistema de estabilização."
...
/obj/item/organ/cyberimp/chest/thrusters/Initialize(mapload)
	. = ..()
	AddComponent( 		/datum/component/jetpack, 		FALSE, 		1.5 NEWTONS, 		COMSIG_THRUSTER_ACTIVATED, 		COMSIG_THRUSTER_DEACTIVATED, 		THRUSTER_ACTIVATION_FAILED, 		CALLBACK(src, PROC_REF(allow_thrust), 0.01), 		CALLBACK(src, PROC_REF(allow_thrust), 0.01), 		/datum/effect_system/trail_follow/ion, 	)
...
/obj/item/organ/cyberimp/chest/spine
	...
	desc = "Esta interface raquidiana gravitrônica é capaz de melhorar o atletismo de um usuário, permitindo-lhes maior capacidade física. Contém um espaço que pode ser atualizado com um núcleo de anomalia de gravidade, melhorando seu desempenho."
...
/obj/item/organ/cyberimp/chest/spine/atlas
	...
	desc = "Esta interface raquidiana gravitrônica é capaz de melhorar o atletismo de um usuário, permitindo-lhes maior capacidade física. Este foi melhorado através da instalação de um núcleo de anomalia de gravidade, permitindo manipulação de gravidade pessoal. Não só você pode andar com os pés plantados firmemente no chão mesmo durante uma perda de gravidade ambiental, mas você também carrega cargas mais pesadas com relativa facilidade."
	icon_state = "herculean_implant_core"
