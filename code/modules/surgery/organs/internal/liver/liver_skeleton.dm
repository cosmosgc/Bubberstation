		...
		if(chem.overdosed)
			if(SPT_PROB(2, seconds_per_tick)) //big oof
				...
				if(bodypart)
					...
					organ_owner.visible_message(span_warning("[organ_owner] rattles loudly and flails around!!"), span_danger("Seus ossos doem tanto que seu espasmo muscular perdido!"))
