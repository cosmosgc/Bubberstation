ADMIN_VERB(reset_tram, R_DEBUG|R_ADMIN, "Reset Tram", "Reset a tram controller or its contents.", ADMIN_CATEGORY_DEBUG)
	...
	if(isnull(broken_controller))
		to_chat(user, span_warning("Não achei um controlador de transporte com identificação.[selected_transport_id]!"))
		return
	...
	switch(reset_type)
		...
		if("Delete Datum")
			var/confirm = tgui_alert(user, "Excluir[selected_transport_id]Será irrecuperável nesta rodada. Tem certeza?", "Delete tram ID [selected_transport_id]", list("Yes", "Cancel"))
			if(confirm != "Yes")
				return
