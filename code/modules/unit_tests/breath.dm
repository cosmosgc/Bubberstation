...
/datum/unit_test/breath/breath_sanity/Run()
	...
	TEST_ASSERT(source.toggle_internals(lab_rat) && !isnull(lab_rat.internal), "Plasmaman toggle_internals() failed to toggle internals")
	lab_rat.breathe()
	TEST_ASSERT(lab_rat.failed_last_breath && lab_rat.has_alert(ALERT_NOT_ENOUGH_OXYGEN), "Os humanos devem ser sufocar de tanques N2 puros.")
...
/datum/unit_test/breath/breath_sanity_plasmamen/Run()
	...
	TEST_ASSERT(source.toggle_internals(lab_rat) && !isnull(lab_rat.internal), "Plasmaman toggle_internals() failed to toggle internals")
	lab_rat.breathe()
	TEST_ASSERT(lab_rat.failed_last_breath && lab_rat.has_alert(ALERT_NOT_ENOUGH_PLASMA), "Os humanos devem ser sufocar de tanques N2 puros.")
