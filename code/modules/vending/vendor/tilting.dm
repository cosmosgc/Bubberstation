...
/atom/movable/proc/fall_and_crush(turf/target, damage, chance_to_crit = 0, forced_crit_case = null, paralyze_time, crush_dir = get_dir(get_turf(src), target), damage_type = BRUTE, damage_flag = MELEE, rotation = 90)
	...
	if (!target.is_blocked_turf(TRUE, src, list(src)))
		for(var/atom/atom_target in (target.contents) + target)
			...
			if (crushed)
				atom_target.visible_message(span_danger("[atom_target]é esmagado por[src]!"), span_userdanger("Você está esmagado por[src]!"))
		...
		visible_message(span_danger("[src]Pontas sobre, batendo forte em[target]!"))
		flags_to_return |= SUCCESSFULLY_FELL_OVER
		post_tilt()
	else
		visible_message(span_danger("[src]Rebotes cômicas como ele não bate em[target]!"))
	...
	switch (crit_case)
		if(CRUSH_CRIT_SHATTER_LEGS) // shatter their legs and bleed 'em
			...
			if(left_leg || right_leg)
				carbon_target.visible_message(span_danger("[carbon_target]As pernas se quebram com uma crise doentia!"), span_userdanger("Suas pernas se quebram com uma crise doentia!"))
			return TRUE
		if(CRUSH_CRIT_PARAPLEGIC) // paralyze this binch
			...
			if (!iscarbon(atom_target))
				return FALSE
			var/mob/living/carbon/carbon_target = atom_target
			visible_message(span_danger("[carbon_target]A medula espinhal está obliterada com uma crise doentia!"), ignored_mobs = list(carbon_target))
		...
		if(CRUSH_CRIT_SQUISH_LIMB) // limb squish!
			...
			carbon_target.visible_message(span_danger("[carbon_target]O corpo está mutilado sob a massa de[src]!"), span_userdanger("Seu corpo está mutilado sob a massa de[src]!"))
			return TRUE
		if(CRUSH_CRIT_HEADGIB) // skull squish!
			...
			if(carbon_head)
				if(carbon_head.dismember())
					carbon_target.visible_message(span_danger("[carbon_head]Explode em um banho de sangue debaixo[src]!"),	span_userdanger("Oh f-"))
...
/obj/machinery/vending/proc/tilt(atom/fatty, local_crit_chance = crit_chance, forced_crit, range = 1)
	...
	if(Adjacent(fatty))
		...
		if (. & SUCCESSFULLY_FELL_OVER)
			visible_message(span_danger("[src]Vira!"))
			tilted = TRUE
...
/obj/machinery/vending/apply_crit_crush(crit_case, atom_target)
	...
	switch (crit_case)
		...
		if (VENDOR_CRUSH_CRIT_PIN) // pin them beneath the machine until someone untilts it
			...
			living_target.visible_message(span_danger("[living_target]está preso por baixo.[src]!"), span_userdanger("Você está preso por[src]!"))
			return TRUE
...
/obj/machinery/vending/proc/untilt(mob/user)
	if(user)
		user.visible_message(span_notice("[user]direitos[src]."), 			span_notice("Você está certo.[src]."))
