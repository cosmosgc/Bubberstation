...
/obj/item/nullrod/rosary/attack(mob/living/target, mob/living/user, params)
	...
	user.visible_message(span_info("[user]De joelhos[target == user ? null : " next to [target]"]e começa a fazer uma oração para[deity_name]."), 		span_info("Você se ajoelha.[target == user ? null : " next to [target]"]E começar uma oração para[deity_name]."))

	praying = TRUE
	if(do_after(user, 5 SECONDS, target = target))
		target.reagents?.add_reagent(/datum/reagent/water/holywater, 5)
		to_chat(target, span_notice("[user]É uma oração para[deity_name]Aliviou sua dor!"))
