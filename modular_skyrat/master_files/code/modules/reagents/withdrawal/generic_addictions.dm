...
/datum/addiction/nicotine/proc/trigger_random_side_effect(mob/living/carbon/affected_carbon, seconds_per_tick, strength = 1)
	switch(rand(1, 4))
		...
		if(2)
			to_chat(affected_carbon, span_warning("Sua cabeça dói."))
			affected_carbon.adjust_stamina_loss(50 * REM * seconds_per_tick)
		if(3)
			if(strength >= 2)
				to_chat(affected_carbon, span_warning("Você se sente um pouco tonto."))
				affected_carbon.set_dizzy(strength * 6 SECONDS)
		if(4)
			if(strength >= 2)
				to_chat(affected_carbon, span_warning("Você se sente cansado."))
