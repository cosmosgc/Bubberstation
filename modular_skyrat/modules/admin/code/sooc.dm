...
/client/verb/sooc(msg as text)
	...
	if(!holder)
		...
		if(!job || !job_lookup[job])
			to_chat(src, span_danger("Você não é um papel de segurança!"))
			return
		if(!GLOB.sooc_allowed)
			to_chat(src, span_danger("SOOC é globalmente mudo."))
			return
...
/proc/toggle_sooc(toggle = null)
	...
	for(var/iterated_listener in listeners)
		var/client/iterated_client = iterated_listener
		to_chat(iterated_client, span_oocplain("<b>O canal SOOC tem sido globalmente[GLOB.sooc_allowed ? "enabled" : "disabled"].</b>"))
