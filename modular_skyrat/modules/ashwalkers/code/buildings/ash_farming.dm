...
/obj/structure/simple_farm/examine(mob/user)
	. = ..()
	. += span_notice("<br>[src]estará pronto para a colheita.[DisplayTimeText(COOLDOWN_TIMELEFT(src, harvest_timer))]")
	if(farm_level < SKILL_LEVEL_LEGENDARY)
		. += span_notice("<br>Você pode usar<b>Adubo de minhoca</b>para aumentar o nível da fazenda.")
	if(stored_worms < 5)
		. += span_notice("<br>Você pode usar<b>Vermes.</b>para ajudar o desenvolvimento da fazenda.")
