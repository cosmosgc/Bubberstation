...
/proc/process_eorg_bans()
	for(var/mob/iterating_player in GLOB.mob_list)
		if(iterating_player.ckey && is_banned_from(iterating_player.ckey, BAN_EORG))
			...
			iterating_player.visible_message(span_notice("[iterating_player]é teletransportado de volta para casa, espero que para uma família sempre amada!"), span_userdanger("Como você está banido, você será deletado."))
