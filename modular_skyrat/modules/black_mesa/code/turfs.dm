...
/turf/open/water/electric/Entered(atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	...
	if(isliving(arrived))
		...
		lightning.start()
		unlucky_mob.visible_message(span_danger("[unlucky_mob.name]está chocado com[src]!"), 		span_userdanger("Você sente um poderoso curso de choque através do seu corpo!"), 		span_hear("Você ouve uma forte rachadura elétrica!"))
