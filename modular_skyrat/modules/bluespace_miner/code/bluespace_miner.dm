...
/obj/machinery/bluespace_miner/examine(mob/user)
	...
	if(mining_stat)
		if(mining_stat & BLUESPACE_MINER_TOO_CLOSE)
			. += span_warning("[src]Está em um ambiente subótimo, muito perto de outra mina azul.")
			return . // This needs relocation to fix so we won't bother with the rest
		if(mining_stat & BLUESPACE_MINER_TOO_HOT)
			. += span_warning("[src]está em um ambiente subótimo:" + span_boldwarning("Temperatura muito alta!"))
		if(mining_stat & BLUESPACE_MINER_LOW_PRESSURE)
			. += span_warning("[src]está em um ambiente subótimo:" + span_boldwarning("Pressão muito baixa!"))
		else if(mining_stat & BLUESPACE_MINER_HIGH_PRESSURE)
			. += span_warning("[src]está em um ambiente subótimo:" + span_boldwarning("Pressão muito alta!"))
