...
/obj/item/weaponcell/examine(mob/user)
	. = ..()
	if(shot_name)
		. += span_noticealien("Usando isso em uma arma baseada em celular vai desbloquear o[shot_name]Modo de disparo")

	if(!toggle_modes) //Doesn't show a description if it can't be toggled in the first place.
		return

	if(medicell_examine)
		. += span_notice("As medidas de segurança no Medicell, evitando danos clones, são[is_toggled ? "enabled" : "disabled"]")
		return
	else
		. += span_notice("[src]está usando o[is_toggled ? "primary" : "secondary"]Modo.")
...
/obj/item/weaponcell/attack_self(mob/living/user)
	...
	if(medicell_examine)
		balloon_alert(user, "segurança[is_toggled ? "enabled" : "disabled"]")
		return
	else if(refresh_shot_name())
		balloon_alert(user, "Pronto para[shot_name]")
