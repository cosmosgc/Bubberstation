...
/obj/item/clockwork/integration_cog/attack_atom(atom/attacked_atom, mob/living/user, params)
	...
	//Insert the cog
	balloon_alert(user, "inserindo[src]...")
	if(!do_after(user, 4 SECONDS, target = cogger_apc))
		balloon_alert(user, "não foi possível inserir[src]!")
		return
	...
	cogger_apc.update_appearance()
	balloon_alert(user, "[src]inserido")
	...
	if(!cogger_apc.clock_cog_rewarded)
		...
		send_clock_message(null, span_brass(span_bold("[user]Instalou uma engrenagem de integração em[cogger_apc].")), msg_ghosts = FALSE)
...
/obj/item/clockwork/integration_cog/proc/finish_setup(obj/machinery/power/apc/cogger_apc)
	...
	send_clock_message(null, span_brass(span_bold("[cogger_apc]A engrenagem de integração terminou a inicialização.")), msg_ghosts = FALSE)
...
/obj/machinery/power/apc/examine(mob/user)
	...
	if(isliving(user))
		...
		if(integration_cog && IS_CLOCK(user))
			. += span_brass("A engrenagem de integração é[integration_cog.set_up ? "fully initialized" : "still initializing"].")
...
/obj/machinery/power/apc/crowbar_act(mob/user, obj/item/crowbar)
	...
	balloon_alert(user, "Investigando algo fora de[src]...")
