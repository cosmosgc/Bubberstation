...
/obj/item/clockwork/trap_placer/proc/place_trap(atom/target, mob/user)
	...
	to_chat(user, span_brass("Seu lugar.[src], use um<b>Placa relojoeira</b>para ligá-lo a outras armadilhas."))
...
/obj/structure/destructible/clockwork/trap/wrench_act(mob/living/user, obj/item/I)
	...
	balloon_alert(user, "Desconectado.[src]")
...
/datum/component/clockwork_trap/proc/on_attackby(datum/source, obj/item/attack_item, mob/user, list/modifiers)
	...
	if(slab.buffer)
		...
		if(takes_input)
			to_chat(user, span_brass("Você se conecta.[slab.buffer.parent]Para[parent]."))
		...
		if(sends_input)
			to_chat(user, span_brass("Você se prepara para se conectar.[parent]com outros dispositivos."))
			slab.buffer = src
