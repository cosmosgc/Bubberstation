/obj/machinery/space_heater/wall_mounted
	name = "mounted heater"
	desc = "Um dispositivo compacto de aquecimento e resfriamento para aplicações em pequena escala, feito para montar em paredes para cima e para fora do caminho. No entanto, como outros aquecedores de ambiente, estes ainda requerem energia celular para funcionar."
...
/obj/item/wallframe/wall_heater/examine(mob/user)
	. = ..()
	if(cell)
		. += span_notice("Ele contém um[ispath(cell) ? cell::name : cell.name], que poderia ser substituído.")
	else
