...
/obj/machinery/arc_furnace
	name = "arc furnace"
	desc = "Um forno de arco, uma máquina especializada que pode rapidamente fundir minérios usando, como o nome indica, enormes quantidades de eletricidade. Embora não tão rápido e eficiente quanto outros métodos de refino de minério, o forno de arco é capaz de retornar<b>Maiores quantidades de material refinado.</b>do que um processo padrão de refino pode. Um adesivo no lado observa que isso pode<b>Gases de escape para o ar</b>durante a operação."
...
/obj/machinery/arc_furnace/examine(mob/user)
	. = ..()
	if(length(contents))
		. += span_notice("Tem.<b>[contents[1]]</b>Sentado nele.")
