...
/obj/vehicle/ridden/wheelchair/hardlight/atom_destruction(damage_flag)
	visible_message(span_notice("[src]Os emissores de luz dura são interrompidos."))
	qdel(src)
	return ..()
...
/obj/vehicle/ridden/wheelchair/hardlight/post_unbuckle_mob()
	...
	visible_message(span_notice("[src]Esvoaça e desaparece à medida que os emissores de luz dura desengatam."))
	qdel(src)
