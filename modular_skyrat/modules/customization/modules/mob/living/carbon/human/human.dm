...
/mob/living/carbon/human/verb/toggle_undies()
	...
	if(stat != CONSCIOUS)
		to_chat(usr, span_warning("Você não pode trocar a visibilidade da cueca agora..."))
		return
	...
	var/picked_visibility = input(src, "Escolha a configuração de visibilidade.", "Mostrar/Esconder roupas íntimas") as null|anything in choice_list
...
/mob/living/carbon/human/proc/mutant_part_visibility(quick_toggle, re_do)
	...
	// Stat check
	if(stat != CONSCIOUS)
		to_chat(usr, span_warning("Não pode fazer isso agora..."))
		return
	...
	// Choice to action
	if(pick == "reveal all")
		to_chat(usr, span_notice("Você não está mais tentando esconder suas partes mutantes."))
		LAZYNULL(try_hide_mutant_parts)
		update_body_parts()
		return

	else if(pick in try_hide_mutant_parts)
		to_chat(usr, span_notice("Você não está mais tentando esconder seu[pick]."))
		LAZYREMOVE(try_hide_mutant_parts, pick)
	else
		to_chat(usr, span_notice("Agora você está tentando esconder seu[pick]."))
...
/mob/living/carbon/human/verb/acting()
	...
	if(duration)
		...
		to_chat(src, "Agora você está fingindo.[impairment].")
...
/mob/living/carbon/human/proc/acting_expiry(impairment)
	if(impairment)
		...
		// Notify the user
		to_chat(src, "Você não está mais fingindo.[impairment].")
