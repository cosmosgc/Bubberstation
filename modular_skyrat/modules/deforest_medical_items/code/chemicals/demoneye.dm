...
/datum/reagent/drug/demoneye
	name = "DemonEye"
	description = "Uma droga de aumento de desempenho originalmente desenvolvida em Marte. Um favorito entre gangues e outros foras da lei no planeta, embora o uso excessivo possa causar terrível vício e danos corporais."
	color = "#af00be"
...
/datum/reagent/drug/demoneye/on_mob_end_metabolize(mob/living/carbon/human/our_guy)
	...
	if(constant_dose_time < CONSTANT_DOSE_SAFE_LIMIT || !our_guy.blood_volume)
		our_guy.visible_message(
				span_danger("[our_guy]Os olhos desvanecem-se de seu mal olhando vermelho de volta ao normal..."),
				span_danger("Sua visão lentamente volta ao normal quando perde sua força não natural...")
		)
	else
		our_guy.visible_message(
			span_danger("[our_guy]Como veias explodem violentamente, espalhando sangue por toda parte!"),
			span_danger("Suas testemunhas estão por causa do estresse!")
		)
