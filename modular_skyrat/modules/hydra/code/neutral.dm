...
/datum/action/innate/hydrareset/Activate()
	...
	hydra.visible_message(span_notice("[hydra.name]empurra todas as três cabeças para frente, parecem estar falando como um coletivo."), 							span_notice("Agora você está falando como[hydra_quirk.original_name]!"), ignored_mobs=owner)
...
/datum/action/innate/hydra/Activate() //Oops, all hydra!
	...
	hydra.visible_message(span_notice("[hydra.name]Puxa o resto de suas cabeças para trás; e coloca[selhead]É para frente."), 							span_notice("Agora você está falando como[selhead]!"), ignored_mobs=owner)
