...
/obj/machinery/power/colony_ore_thumper
	name = "ore thumper"
	desc = "Um quadro com um bloco pesado de metal suspenso em cima de um tubo. Deve ser colocado ao ar livre e dada uma conexão de energia com fio. Forças pressurizaram o gás no solo que traz recursos enterrados."
...
/obj/machinery/power/colony_ore_thumper/examine(mob/user)
	...
	if(!istype(get_turf(src), /turf/open/misc))
		. += span_notice("Deve ser construído sobre<b>Terreno adequado</b>Como se chama?")
	. += span_notice("Deve ter um motor,<b>Conexão com fio</b>Correndo por baixo com<b>[active_power_usage / BASE_MACHINE_ACTIVE_CONSUMPTION]kW</b>de excesso de energia para funcionar.")
	. += span_notice("Ele produzirá uma caixa de materiais depois de bater.[slam_jams_needed]Tempos.")
	. += span_notice("Atualmente, ele Bateu[slam_jams] / [slam_jams_needed]Tempos necessários.")
	. += span_notice("Ele vai parar de produzir recursos se você quiser<b>Muitas pilhas de minério.</b>Perto dele.")
	. += span_notice("O batedor não pode funcionar se for<b>Muito bem, muito bem.</b>, precisando<b>Pelo menos.[ore_spawn_range]espaços</b>em todas as direções entre ele outro batedor.")
...
/obj/machinery/power/colony_ore_thumper/attack_hand(mob/user, list/modifiers)
	...
	to_chat(user, span_notice("Você comuta[src]O botão de energia."))
