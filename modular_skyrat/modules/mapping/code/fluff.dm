...
/obj/structure/decorative/fluff/ai_node/take_damage()
	...
	if(atom_integrity >= 50)	//breaks it a bit earlier than it should, but still takes a few hits to kill it
		...
		visible_message(span_notice("[src] sparks and explodes! You hear a faint, buzzy scream..."), span_hear("Você ouve um som alto, seguido de um grito fraco e barulhento."))
