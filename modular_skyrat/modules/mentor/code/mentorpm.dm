...
/client/proc/cmd_mentor_pm_panel() // We're not using this and I'm debating removing the code as it's dead and useless. We don't need mentors PMing people out of the blue. That's not really how we operate.
	...
	var/target = input(src, "A quem nos envia uma mensagem?", "Mentor PM", null) in sorted|null
...
/client/proc/cmd_mentor_pm(whom, msg)
	...
	if(is_mentor(whom))
		to_chat(GLOB.mentors, span_purple(span_mentor("[src]começou a responder[whom]É a Mhelp.")))
	...
	if(!msg)
		...
		if(!msg)
			if (is_mentor(whom))
				to_chat(GLOB.mentors, span_mentor(span_purple("[src]Parou a resposta deles para[whom]É a Mhelp.")))
			return
	...
	if(target.is_mentor())
		if(is_mentor())//both are mentors
			to_chat(target, span_mentor(span_purple("Mentor PM de...<b>[key_name_mentor(src, target, TRUE, FALSE, FALSE)]</b>: [msg]")))
			to_chat(src, span_mentor(span_blue("Mentor PM para...<b>[key_name_mentor(target, target, TRUE, FALSE, FALSE)]</b>: [msg]")))

		else		//recipient is a mentor but sender is not
			to_chat(target, span_mentor(span_purple("Responde, PM de...<b>[key_name_mentor(src, target, TRUE, FALSE, show_char)]</b>: [msg]")))
			to_chat(src, span_mentor("Mentor PM para...<b>[key_name_mentor(target, target, TRUE, FALSE, FALSE)]</b>: [msg]"))

	else
		if(is_mentor())	//sender is a mentor but recipient is not.
			to_chat(target, span_mentor(span_purple("Mentor PM de...<b>[key_name_mentor(src, target, TRUE, FALSE, FALSE)]</b>: [msg]")))
			to_chat(src, span_mentor("Mentor PM para...<b>[key_name_mentor(target, target, TRUE, FALSE, show_char)]</b>: [msg]"))
