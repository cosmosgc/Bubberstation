...
/obj/item/nifsoft_remover
	...
	special_desc = "Dada a relativamente recente e súbita proliferação de NIFs, seu uso no crime tanto mesquinho quanto organizado disparou nos últimos anos. A existência de comunicações baseadas em nanomáquinas que não podem ser efetivamente monitoradas ou hackeadas deu à maioria dos PMCs causaram preocupação suficiente para inventar seus próprios dispositivos. Este é um modelo \"Wrangler\" NIF-Cutter, usado para limpar programas diretamente do Framework de um usuário."
...
/obj/item/nifsoft_remover/attack(mob/living/carbon/human/target_mob, mob/living/user)
	...
	if(!target_nif || !length(target_nif.loaded_nifsofts))
		balloon_alert(user, "[target_mob]Não tem NIFSofts!")
		return
	...
	user.visible_message(span_warning("[user]começa a usar[src]Vamos.[target_mob]"), span_notice("Você começa a usar[src]Vamos.[target_mob]"))
	...
	to_chat(user, span_notice("Você remove com sucesso.[nifsoft_to_remove]."))
...
/obj/item/nif_repair_kit
	...
	special_desc = "Os efeitos do capitalismo e da indústria são profundos, e eles funcionam dentro da indústria Nanite Implant Framework também. Frameworks, dispositivos complicados como eles são, normalmente estão bloqueados no nível de firmware para exigir marcas específicas 'aprovadas' de pasta de reparo ou docas de reparo. Este kit hackeado foi desenvolvido pelo Coven Altspace como uma alternativa de freeware, espalhado por todo o espaço extra-terrânico para qualidade de vida para usuários localizados nas periferias da sociedade."
	icon = 'modular_skyrat/modules/modular_implants/icons/obj/devices.dmi'
...
/obj/item/nif_repair_kit/attack(mob/living/carbon/human/mob_to_repair, mob/living/user)
	...
	if(!installed_nif)
		balloon_alert(user, "[mob_to_repair]Falta um NIF")
	...
	to_chat(user, span_notice("Você reparou com sucesso.[mob_to_repair]É NIF"))
	to_chat(mob_to_repair, span_notice("[user]Conserta com sucesso o seu NIF"))
...
/obj/item/nif_hud_adapter/examine(mob/user)
	...
	if(length(compatible_glasses_names))
		. += span_cyan("\nEste item vai funcionar com os seguintes óculos:[english_list(compatible_glasses_names)].")
...
/obj/item/nif_hud_adapter/interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	...
	user.visible_message(span_notice("[user]upgrades[target_glasses]Com[src]."), span_notice("Você se atualiza.[target_glasses]para ser NIF HUD compatível."))
