...
/obj/item/clothing/mask/gas/bdsm_mask/proc/toggle(mob/living/carbon/user)
	mask_on = !mask_on
	to_chat(user, span_notice("Você gira o filtro de ar[mask_on ? "on. Use with caution!" : "off. Now it's safe to wear."]"))
...
/obj/item/clothing/mask/gas/bdsm_mask/proc/toggle_gag(user)
	...
	to_chat(user, span_notice("Você.[speech_disabled ? "disable" : "enable"]A mordaça na máscara."))
	update_mob_action_buttonss()
	update_icon()
