...
/obj/item/clothing/erp_leash/attack(mob/living/to_be_leashed, mob/living/user, params)
	...
	if(!to_be_leashed.check_erp_prefs(/datum/preference/toggle/erp/sex_toy, user, src))
		to_chat(user, span_danger("[to_be_leashed]Não quer que faça isso."))
		return
	/// Actually start the leashing part here
	to_be_leashed.visible_message(span_warning("[user]Levanta o[src]Para[to_be_leashed]O pescoço!"),				span_userdanger("[user]começa a trazer o[src]No seu pescoço!"),				span_hear("Você ouve um clique de luz enquanto a pressão aumenta no ar ao redor do pescoço."))
...
/obj/item/clothing/erp_leash/proc/create_leash(mob/user, mob/ouppy)
	...
	if(our_leash_component.resolve()) // The component will immediately delete itself if there's an existing one; this sanity checks for feedback on if it failed.
		...
		return
	else to_chat(user, span_danger("Há uma coleira presa a[ouppy]Já."))
...
/datum/component/leash/erp/proc/on_item_attack_self(datum/source, mob/user)
	...
	if(istype(source, /obj/item/clothing/erp_leash))
		...
		if(istype(parent, /mob/living))
			...
			yoinked.visible_message(span_warning("[yoinked]é puxado como[user]Reboca o[source]!"),					span_userdanger("[user]De arrependimento puxa o[source], puxando você mais perto!"),					span_userdanger("Um puxão repentino contra seu pescoço te puxa à frente!"))
...
/datum/component/leash/erp/proc/do_resist(datum/source, mob/user)
	if(istype(parent, /mob) && istype(owner,/obj/item))
		...
		our_parent.visible_message(span_warning("[our_parent]Tentativa de soltar o Gancho.[our_parent.p_them()]Eu mesmo da coleira!"), 			span_userdanger("Você começa a se soltar da coleira..."), 			span_userdanger("Você se mete no escuro, olhando para soltar a coleira..."))
