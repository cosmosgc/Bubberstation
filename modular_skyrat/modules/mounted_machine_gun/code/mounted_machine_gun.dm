...
/obj/machinery/mounted_machine_gun/examine(mob/user)
	. = ..()
	if(ammo_box)
		. += span_notice("Tem.[ammo_box]Carregado, com[ammo_box.ammo_count()]As balas restantes.")
	else
		. += span_danger("Não tem uma caixa de munição carregada!")
	. += span_notice("A capa é[cover_open ? "open" : "closed"]. <b>Alt+click</b>para[cover_open ? "close" : "open"]Ele.")
