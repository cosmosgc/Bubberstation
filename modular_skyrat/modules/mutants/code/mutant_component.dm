...
/datum/component/mutant_infection/process(seconds_per_tick)
	...
	if(host.stat != DEAD)
		return
	if(!ismutant(host))
		to_chat(host, span_cult_large("Você pode sentir seu coração parando, mas algo não está certo...\
A vida não abandonou sua forma quebrada. Você só pode sentir uma fome profunda e imutável que\
Nem mesmo a morte pode parar, você se levantará novamente!"))
	var/revive_time = rand(REVIVE_TIME_LOWER, REVIVE_TIME_UPPER)
...
/datum/component/mutant_infection/proc/transform_host()
	...
	host.visible_message(span_danger("[host] suddenly convulses, as [host.p_they()][stand_up ? " stagger to [host.p_their()] feet and" : ""] gain a ravenous hunger in [host.p_their()] eyes!"), span_alien("Seu caçador!"))
...
/datum/component/mutant_infection/proc/regenerate()
	...
	to_chat(host, span_notice("Você sente uma coceira, tanto dentro como dentro.\
Do lado de fora como seus tecidos tricô e reknit."))
