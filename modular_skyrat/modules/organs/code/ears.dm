/obj/item/organ/ears/teshari
	name = "teshari ears"
	desc = "Um conjunto de quatro longas orelhas de coelho, a principal ferramenta de um Teshari enquanto caça. Naturalmente extremamente sensível a sons altos."
...
/datum/action/cooldown/spell/teshari_hearing
	...
	desc = "Levantem os ouvidos para ouvir sons silenciosos, úteis para ouvir sussurros."
...
/datum/action/cooldown/spell/teshari_hearing/cast(list/targets, mob/living/carbon/human/user = usr)
	...
	user.visible_message(span_notice("[user], pica para cima[user.p_their()]Quatro orelhas, cada uma se contorcendo atentamente!"), span_notice("Você anima os quatro ouvidos, caçando até os sons mais silenciosos."))
...
/datum/action/cooldown/spell/teshari_hearing/proc/teshari_hearing_deactivate(mob/living/carbon/human/user) //Called when you activate it again after casting the ability-- turning them off, so to say.
	...
	user.visible_message(span_notice("[user]Gotas[user.p_their()]Ouvir um pouco, não ouvir de perto."), span_notice("Você baixa os ouvidos, não presta mais atenção."))
...
/datum/status_effect/teshari_hearing/get_examine_text()
	return span_notice("[owner.p_They()] [owner.p_have()] [owner.p_their()]ouvidos animados, ouvindo de perto sons silenciosos.")
