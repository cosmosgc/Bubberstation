...
/datum/component/reagent_clothing/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
	examine_list += span_notice("[parent_clothing]é capaz de ser injetado com um químico em uma forja reagente!")
...
/datum/component/reagent_weapon/proc/on_examine(datum/source, mob/user, list/examine_list)
	SIGNAL_HANDLER
	examine_list += span_notice("[parent_weapon]é capaz de ser imbuído com um produto químico em uma forja reagente!")
