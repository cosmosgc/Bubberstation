...
/obj/item/clothing/head/hair_tie/examine(mob/user)
	. = ..()
	if(picked_hairstyle)
		. += span_notice("Usá-lo mudará seu penteado para[picked_hairstyle]'.")
	. += span_notice("<b>Use na mão</b>Para escolher um novo Penteado.")
	. += span_notice("<b>Alt-click</b> [src]Para atirar.")
...
/obj/item/clothing/head/hair_tie/equipped(mob/living/carbon/human/user, slot)
	...
	user.visible_message(
		span_notice("[user.name]Amarrem-se.[user.p_their()]Cabelo."),
...
/obj/item/clothing/head/hair_tie/dropped(mob/living/carbon/human/user)
	...
	if(!picked_hairstyle || !actual_hairstyle)
		return
	user.visible_message(
		span_notice("[user.name]Toma.[src]Fora[user.p_their()]Cabelo."),
...
/obj/item/clothing/head/hair_tie/click_alt(mob/living/user)
	...
	user.visible_message(
		span_danger("[user.name]coloca[src]Por aí.[user.p_their()]Dedos, começando a apertar!"),
		span_notice("Você tenta filmar[src]!"),
	)
	flick_hair_tie(user)
