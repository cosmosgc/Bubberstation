...
/obj/item/melee/breaching_hammer/proc/breaching_loop(mob/living/user, obj/target)
	while(!user.stat && target && user.Adjacent(target) && target.get_integrity() >= 1)
		...
		target.visible_message(span_warning("[target]Começa a cair e se deformar com cada golpe de[src]!"), span_warning("Estamos invadindo.[target]."), 		span_hear("Você ouve um som de metal contra metal."))
