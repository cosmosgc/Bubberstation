...
/obj/structure/ore_vein/attackby(obj/item/W, mob/user, params)
	...
	if(!ore_type)
		...
		return FALSE
	if(!ore_amount)
		to_chat(user, span_notice("O[src]é muito baixa qualidade para produzir qualquer quantidade útil de[ore_descriptor]."))
		return FALSE
	if(depleted == TRUE)
		to_chat(user, span_notice("Esta veia de minério está exausta."))
		return FALSE
	// Our early return checks to tell the user what went wrong.
	to_chat(user, span_notice("Você começa a minar o[ore_descriptor]..."))
	if(W.use_tool(src, user, src.mining_time, volume=50))
		to_chat(user, span_notice("Você é meu.[ore_descriptor]."))
