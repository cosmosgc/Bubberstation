...
/datum/species/synthetic/create_pref_unique_perks()
	...
	perk_descriptions += list(list( //tryin to keep traits minimal since synths will get a lot of traits when my upstream traits pr is merged
		...
		SPECIES_PERK_DESC = "Ao contrário dos orgânicos, você não explode diante de um vácuo! Além disso, seu chassi é construído com tanta força que lhe dá imunidade à superpressão! Apenas certifique-se que o frio extremo ou calor não frite seus circuitos."
	))
	...
	perk_descriptions += list(list(
		...
		SPECIES_PERK_DESC = "[plural_form]Não podem ser descascados, decepcionantes metamorfos em toda a galáxia.",
	))
	...
	perk_descriptions += list(list(
		...
		SPECIES_PERK_DESC = "[plural_form]são incapazes de obter nutrição de alimentos tradicionais. Em vez disso, você deve consumir combustível de solda ou estender um fio de seu braço para extrair energia de um APC. Além disso, soldadores e fios são suas suturas e malhas e só substâncias químicas específicas metabolizam dentro de você. Isso varia de uísque, a sintanol, a vários medicamentos obscuros. Finalmente, sofre de feridas exclusivas de sintéticos."
	))
	...
	perk_descriptions += list(list(
		...
		SPECIES_PERK_DESC = "[plural_form]pode cantar tons musicais usando um sintetizador interno.",
	))
	...
	perk_descriptions += list(list(
		...
		SPECIES_PERK_DESC = "[plural_form]são fracos para objetos bruscos.",
	))
