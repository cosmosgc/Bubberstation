...
/obj/item/wargame_projector/examine(mob/user)
	. = ..()
	if(projections)
		. += span_notice("Ele está mantendo atualmente<b>[projections.len]/[max_signs]</b>Projeções.")
	...
	. += span_warning("<b>Controlem-se!</b>O projetor permitirá que você limpe todos os hologramas ativos.")
...
/obj/item/wargame_projector/click_alt(mob/user)
	...
	balloon_alert(user, "Um cor mudou.")
	set_greyscale(holosign_color)
	return CLICK_ACTION_SUCCESS

/obj/item/wargame_projector/item_ctrl_click(mob/user)
	if(tgui_alert(user,"Todos os hologramas ativos?", "Hologram Removal", list("Yes", "No")) == "Yes")
