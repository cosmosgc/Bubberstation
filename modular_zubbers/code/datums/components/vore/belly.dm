...
/obj/vore_belly/Entered(atom/movable/arrived, atom/old_loc, list/atom/old_locs)
	...
	to_chat(living_parent, span_notice("[arrived]Desliza para o seu[LOWER_TEXT(name)]."))
	...
	if(ismob(arrived))
		...
		deep_search_prey(M)
		to_chat(M, boxed_message("Você desliza para dentro[span_notice("[owner.parent]")]'s[span_green(LOWER_TEXT(name))]!\n[EXAMINE_SECTION_BREAK][format_message(desc, M)]"))
		// Add the appearance_holder to prey so they can see fellow prey
		if(can_taste && iscarbon(M))
			var/mob/living/carbon/H = M
			to_chat(living_parent, span_notice("[H]gostos de[H.dna.features["taste"] || "nothing"]."))
		if(M.client)
...
/obj/vore_belly/container_resist_act(mob/living/user)
	...
	if(escapable(user))
		if(prob(escape_chance))
			...
			to_chat(living_parent, span_warning("Sua presa parece ser incapaz de fazer qualquer progresso em escapar de seu[LOWER_TEXT(name)]."))
...
/obj/vore_belly/proc/release(atom/movable/AM)
	...
	AM.visible_message(span_warning("[living_parent] [LOWER_TEXT(release_verb)] [AM]de sua[LOWER_TEXT(name)]."), pref_to_check = /datum/preference/toggle/erp/vore_enable)
