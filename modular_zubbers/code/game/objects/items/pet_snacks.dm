...
/obj/item/pet_food/pet_space_treat/attack(mob/living/basic/target_pet, mob/user)
	...
	target_pet.desc += span_notice("\n[target_pet.p_They()]Parece[target_pet.p_s()]Mais forte contra o vazio do espaço.")
	to_chat(user, span_notice("Você alimenta o deleite para o[target_pet], que rapidamente devoram."))
	qdel(src)
	return
