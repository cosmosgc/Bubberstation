...
/obj/item/harmalarm/bubbers
	desc = "Libera uma explosão inofensiva que confunde a maioria dos orgânicos. Use em caso de baleias azuis hostis."
...
/obj/item/harmalarm/bubbers/click_alt(mob/living/user)
	...
	if(!str)
		...
		return
	if(length(str) <= 2)
		to_chat(user, span_warning("Muito curto!"))
		return
	alarm_phrase = str
	to_chat(user, span_notice("Você ajustou a frase de alarme para '[str]'."))
...
/obj/item/harmalarm/bubbers/attack_self(mob/user)
	...
	if(safety == TRUE)
		user.visible_message("<font color='red' size='2'>[user]Blares uma sirene quase surda de seus alto-falantes!</font>", 			span_userdanger("Sua sirene brilha ao redor[iscyborg(user) ? "you" : "and confuses you"]!"), 			span_danger("A sirene perfura sua audição!"))
