...
#define PORTAL_SIGNAL_LIST list( 	COMSIG_MOB_POST_EQUIP, 	COMSIG_HUMAN_UNEQUIPPED_ITEM, 	COMSIG_HUMAN_TOGGLE_UNDERWEAR, 	COMSIG_MOB_HANDCUFFED, 	COMSIG_MOB_EMOTE, 	COMSIG_EMOTE_OVERLAY_EXPIRE, 	COMSIG_HUMAN_ADJUST_AROUSAL, 	COMSIG_HUMAN_TOGGLE_AROUSAL, 	COMSIG_HUMAN_TOGGLE_GENITALS )
...
/obj/structure/lewd_portal/Destroy()
	visible_message("[src]Desaparece!")
...
/obj/structure/lewd_portal/examine(mob/user)
	...
	. += span_notice("Está atualmente em[inspect_mode]Modo.")
...
/obj/structure/lewd_portal/user_buckle_mob(mob/living/M, mob/user, check_loc)
	if(!M.check_erp_prefs(/datum/preference/toggle/erp/sex_toy, user, src))
		to_chat(user, span_danger("Parece que sim.[M]Não quer que faça isso."))
		return FALSE
	if (!ishuman(M))
		balloon_alert(user, "[M.p_they()]Não cabe!")
		return FALSE
	if(portal_mode == GLORYHOLE)
	...
	if (!isnull(linked_portal.current_mob))
		...
		return FALSE
	visible_message("[user]slots[M]para o[src]!")
...
/obj/structure/lewd_portal/post_unbuckle_mob(mob/living/unbuckled_mob)
	...
	visible_message("[current_mob]Sai o[src]")
	current_mob = null
...
/obj/lewd_portal_relay/Destroy(force)
	...
	visible_message("[src]Desaparece para o portal!")
...
/obj/lewd_portal_relay/attack_hand_secondary(mob/living/user)
	...
	if(dir == NORTH)
		...
		dir = NORTH
	to_chat(user, span_info("Você vira\the [name]Câmbio."))
