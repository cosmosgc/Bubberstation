...
/obj/machinery/research/anomaly_refinery
	desc = "Uma máquina avançada equipada com software de previsão de bombas de última geração que é capaz de implosão-comprimindo núcleos de anomalia crus em artefatos acabados. Vem com uma estabilização integrada e um campo supressor de som que consome muita energia para evitar tremores detestáveis e sons ouvidos em toda a estação, como para não acordar qualquer usuário de dormitórios."
...
/obj/machinery/research/anomaly_refinery/examine(mob/user)
	...
	. += span_notice("Requer<b>[display_power(REFINERY_ANOMALY_POWER_REQUIREMENT)]</b>De energia na rede para usar.")
	. += span_notice("Cada refinamento do núcleo leva<b>[DisplayTimeText(REFINERY_ANOMALY_REFINEMENT_TIME,1)]</b>Para completar.")
