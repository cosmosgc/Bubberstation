/datum/bloodsucker_clan/malkavian
	...
	join_description = "Completamente insano. Você ganha alucinações constantes, torna-se um profeta com divagações ininteligíveis, e torna-se o executor do código mascarado."
...
/datum/bloodsucker_clan/malkavian/proc/on_bloodsucker_broke_masquerade(datum/source, datum/antagonist/bloodsucker/masquerade_breaker)
	SIGNAL_HANDLER
	to_chat(bloodsuckerdatum.owner.current, span_userdanger("[masquerade_breaker.owner.current]Quebrou a máscara! Certifique-se.[masquerade_breaker.owner.current.p_they()] [masquerade_breaker.owner.current.p_are()]Eliminado a todo custo!"))
