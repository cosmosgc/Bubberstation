...
/datum/bloodsucker_clan/ventrue
	...
	join_description = "Perder a capacidade de beber de mobs sem cérebro, não pode nivelar ou ganhar novos poderes, em vez disso você criar um fantasma em um Bloodsucker."
...
/datum/bloodsucker_clan/ventrue/spend_rank(datum/antagonist/bloodsucker/source, cost_rank = TRUE, blood_cost)
	if(has_enough_abilities())
		to_chat(bloodsuckerdatum.owner.current, span_danger("Você só pode ter até[VENTRUE_MAX_POWERS]Poderes, qualquer coisa mais deve ser para gastar em seu Ghoul Favorito através de uma Rack Persuasão."))
		return FALSE
	. = ..()
...
/datum/bloodsucker_clan/ventrue/interact_with_ghoul(datum/antagonist/bloodsucker/source, datum/antagonist/ghoul/favorite/ghouldatum)
	...
	if(!istype(ghouldatum))
		return FALSE
	to_chat(bloodsuckerdatum.owner.current, span_warning("Você deseja classificar[ghouldatum.owner.current]Levantar?"))
	to_chat(bloodsuckerdatum.owner.current, span_warning("Isto vai usar[bloodsuckerdatum.GetUnspentRank() >= 1 ? "a unspent Rank" : "[bloodsuckerdatum.get_level_cost()] Blood Thickening Points"]!"))
	...
	if(rank_response == "Yes")
		if(!bloodsuckerdatum.GetUnspentRank() >= 1 && !source.blood_level_gain(FALSE))
			to_chat(bloodsuckerdatum.owner.current, span_danger("Você não tem nenhum nível ou ponto de espessamento de sangue suficiente para o Rank.[ghouldatum.owner.current]Levante-se."))
			return FALSE
...
/datum/bloodsucker_clan/ventrue/proc/ghoul_level(datum/antagonist/ghoul/favorite/ghouldatum)
	...
	if(!ghouldatum.BuyPower(choice, ghouldatum.bloodsucker_powers))
		bloodsuckerdatum.owner.current.balloon_alert(bloodsuckerdatum.owner.current, "[target]Já sabe.[power_name]!")
		return FALSE
	bloodsuckerdatum.owner.current.balloon_alert(bloodsuckerdatum.owner.current, "ensinada[power_name]!")
	to_chat(bloodsuckerdatum.owner.current, span_notice("Você ensinou.[target]Como usar[power_name]!"))

	target.balloon_alert(target, "Aprendi.[power_name]!")
	to_chat(target, span_notice("Seu mestre te ensinou a usar[power_name]!"))
...
/datum/bloodsucker_clan/ventrue/proc/bloodsuckerify(datum/antagonist/ghoul/favorite/ghouldatum)
	...
	switch(stage)
		...
		if(2)
			to_chat(target, span_notice("Sente o sangue do seu Mestre te fortaleza."))
			if(ishuman(target))
