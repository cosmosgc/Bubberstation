...
/datum/action/cooldown/bloodsucker/targeted/lunge/proc/lunge_end(atom/hit_atom, turf/target_turf)
	...
	user.balloon_alert(user, "Você se atira em[target]!")
	...
	if(target.stat == DEAD)
		...
		user.visible_message(
			span_warning("[user]Lágrimas dentro[target]'s[bodypart]!"),
			span_warning("Você rasga em[target]'s[bodypart]!"))
		...
		if(!myheart_now)
			...
			if(!length(organs))
				to_chat(user, span_warning("[target]Não tem órgãos em[bodypart]!"))
