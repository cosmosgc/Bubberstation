	...
	if(target_mob.stat != CONSCIOUS && level_current >= DOMINATE_GHOULIZE_LEVEL)
		if(user.Adjacent(target))
			...
			if(IS_GHOUL(target_mob))
				owner.balloon_alert(owner, "Muito tempo para reviver!")
			else
				owner.balloon_alert(owner, "Muito tempo para Ghoul!")
		return TRUE
	return ..()
...
/datum/action/cooldown/bloodsucker/targeted/mesmerize/dominate/proc/attempt_ghoulize(mob/living/target, mob/living/user)
	...
	if(ghoul?.master == bloodsuckerdatum_power)
		...
		to_chat(user, span_warning("Nós revivemos[target]!"))
	...
	user.balloon_alert(target, "Apenas[DisplayTimeText(living_time)]Deixado para viver!")
	to_chat(target, span_warning("Você só viverá por[DisplayTimeText(living_time)]Obedeça seu mestre e saia em uma chama de glória!"))
