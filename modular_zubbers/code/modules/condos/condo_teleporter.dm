...
/obj/machinery/cafe_condo_teleporter/proc/prompt_and_check_in(mob/user, mob/target)
	...
	if(requested_condo > SHORT_REAL_LIMIT)
		to_chat(target, span_warning("Esta rede só está ligada a[SHORT_REAL_LIMIT]quartos!"))
		return
...
/obj/machinery/cafe_condo_teleporter/proc/check_target_eligibility(mob/to_be_checked)
	if(!Adjacent(to_be_checked))
		to_chat(to_be_checked, span_warning("Você está muito longe\the [src]Para entrar!"))
		return FALSE
	if(to_be_checked.incapacitated)
		to_chat(to_be_checked, span_warning("Você não é capaz de ativar\the [src]Mais!"))
		return FALSE
	return TRUE
