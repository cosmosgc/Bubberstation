...
/datum/action/cooldown/fly/proc/enable_fly()
	...
	owner.plane += 2
	enabled = TRUE
	owner.visible_message(span_notice("[owner.name], takes off over the ground."), span_notice("Você vai voar."))
...
/datum/action/cooldown/fly/proc/disable_fly()
	...
	enabled = FALSE
	owner.visible_message(span_notice("[owner.name], has landed on the ground."), span_notice("Você pousa no chão."))
	playsound(owner, 'sound/effects/gravhit.ogg', 25)
