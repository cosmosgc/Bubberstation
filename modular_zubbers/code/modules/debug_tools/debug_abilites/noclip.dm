...
/datum/action/cooldown/noclip/proc/enable_noclip()
	...
	owner.visible_message(span_notice("[owner.name]Se dissolve no ar. Tornando-se não-físico."), span_notice("Você vai noclip."))
...
/datum/action/cooldown/noclip/proc/disable_noclip()
	...
	owner.visible_message(span_notice("[owner.name], torna-se físico novamente."), span_notice("Você voltou ao normal de novo."))
