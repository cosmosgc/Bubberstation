...
/obj/item/physic_manipulation_tool/examine(mob/user)
	. = ..()
	. += span_notice("Expresso manual:")
	. += span_notice("Use ALT + LMB sem dispositivo para escola cor.")
	. += span_notice("Use LMB no objetivo para arrastar o objetivo.")
	. += span_green("Use RMB enquanto arrasta para congelar o objetivo.")
	. += span_red("Use o LMB para abrir para libertar o objeto.")
	. += span_notice("Use CTRL + LMB enquanto arrasta para jogar o objetivo.")
	. += span_notice("Use ALT + LMB para descobrir o objetivo.")
...
/obj/item/physic_manipulation_tool/proc/on_clicked(atom/source, atom/clicked_on, modifiers)
	...
	if(LAZYACCESS(modifiers, RIGHT_CLICK))
		if(!advanced)
			physgun_user.balloon_alert(physgun_user, "Insuficiência de energia!")
			return
