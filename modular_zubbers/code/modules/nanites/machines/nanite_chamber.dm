/obj/machinery/nanite_chamber
	name = "nanite chamber"
	desc = "Um dispositivo que pode escapar, reprogramar e injetar nanites."
...
/obj/machinery/nanite_chamber/examine(mob/user)
	...
	if(in_range(user, src) || isobserver(user))
		. += span_notice("A exibição de status diz: módulo de varredura foi atualizado para o nível<b>[scan_level]</b>.")
...
/obj/machinery/nanite_chamber/mouse_drop_receive(atom/target, mob/user, params)
	...
	if(close_machine(target))
		log_combat(user, target, "inserido", null, "into [src].")
