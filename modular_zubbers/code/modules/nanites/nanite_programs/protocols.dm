...
/datum/nanite_program/protocol/kickstart
	...
	desc = "Protocolo de Replicação: os nanites focam no crescimento precoce, aumentando fortemente a taxa de replicação por alguns minutos após a implantação inicial, resultando em um volume adicional de 420 nanites sendo produzido durante os dois primeiros minutos."
	use_rate = 0
...
/datum/nanite_program/protocol/factory
	...
	desc = "Protocolo de Replicação: os nanites constroem uma matriz de fábrica dentro do hospedeiro, aumentando gradualmente a velocidade de replicação ao longo do tempo, concedendo um máximo de 2 produção adicional de nanites após aproximadamente 17 minutos. A fábrica decai se o protocolo não estiver ativo, ou se os nanites forem interrompidos por choques ou PEMs."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_REPLICATION
...
/datum/nanite_program/protocol/pyramid
	...
	desc = "Protocolo de Replicação: os nanites implementam um protocolo de replicação cooperativa alternativo que está ativo desde que o nível de saturação nanite esteja acima de 80%, resultando em uma produção de volume adicional de 1,2 por segundo."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_REPLICATION
	var/boost = 1.2
...
/datum/nanite_program/protocol/offline
	...
	desc = "Protocolo de Replicação: enquanto o hospedeiro está dormindo ou de outra forma inconsciente, os nanites exploram a interferência reduzida para se replicar mais rapidamente."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_REPLICATION
	var/boost = 3
...
/datum/nanite_program/protocol/hive
	name = "Hive Protocol"
	desc = "Protocolo de armazenamento: os nanites usam um arranjo de grade mais eficiente para armazenamento de volume, aumentando o volume máximo para 750."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
...
/datum/nanite_program/protocol/zip
	name = "Zip Protocol"
	desc = "Protocolo de armazenamento: os nanites são desmontados e compactados quando não utilizados, aumentando o volume máximo para 1000. No entanto, o processo diminui sua taxa de replicação ligeiramente."
	use_rate = 0.2
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_STORAGE
...
/datum/nanite_program/protocol/free_range
	...
	desc = "Protocolo de armazenamento: os nanites descartam seus protocolos de armazenamento padrão em favor de uma abordagem mais barata e orgânica. Reduz o volume máximo para 250, mas aumenta a taxa de replicação em 0,5."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_STORAGE
	var/boost = 0.5
...
/datum/nanite_program/protocol/unsafe_storage
	name = "S.L.O. Protocol"
	desc = "Protocolo de Armazenamento: S.L.O.P., ou Protocolo de Sobreposição de Nível de Armazenamento, desativa completamente as medidas de segurança normalmente presentes nos nanites, permitindo que atinjam um volume máximo de 2000 mas sob o risco de causar danos ao hospedeiro em concentrações de nanites acima do limite padrão de 500."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_STORAGE
