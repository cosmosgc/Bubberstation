...
/datum/nanite_program/protocol/kickstart
	...
	desc = "Protocolo de Replicação: os nanites focam no crescimento precoce, aumentando fortemente a taxa de replicação por alguns minutos após a implantação inicial,\
resultando em um volume adicional de 420 nanites sendo produzido durante os primeiros dois minutos."
	use_rate = 0
...
/datum/nanite_program/protocol/factory
	...
	desc = "Protocolo de Replicação: os nanites constroem uma matriz de fábrica dentro do hospedeiro, aumentando gradualmente a velocidade de replicação ao longo do tempo,\
concedendo um máximo de 2 produção adicional de nanites após aproximadamente 17 minutos.\
A fábrica decai se o protocolo não estiver ativo, ou se os nanites forem interrompidos por choques ou PEMs."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_REPLICATION
...
/datum/nanite_program/protocol/pyramid
	...
	desc = "Protocolo de Replicação: os nanites implementam um protocolo alternativo de replicação cooperativa que está ativo enquanto o nível de saturação de nanites estiver acima de 80%,\
resultando em uma produção de volume adicional de 1,2 por segundo."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_REPLICATION
	var/boost = 1.2
...
/datum/nanite_program/protocol/unsafe_storage
	name = "S.L.O. Protocol"
	desc = "Protocolo de armazenamento: S.L.O.P., ou Protocolo de substituição de nível de armazenamento, desativa completamente as medidas de segurança normalmente presentes em nanites,\
Permitindo que atinjam um volume máximo de 2000, mas com o risco de causar danos ao hospedeiro em concentrações nanitas acima do limite padrão de 500."
	use_rate = 0
	rogue_types = list(/datum/nanite_program/necrotic)
	protocol_class = NANITE_PROTOCOL_STORAGE
