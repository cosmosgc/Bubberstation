...
/datum/nanite_program/suffocating/active_effect()
	...
	if(prob(1))
		to_chat(host_mob, span_warning("Sente-se."))
...
/datum/nanite_program/skin_decay/active_effect()
	...
	if(prob(5)) //itching
		...
		host_mob.visible_message("[can_scratch ? span_warning("[host_mob] scratches [host_mob.p_their()] [bodypart.name].") : ""]",		span_warning("Sua[bodypart.name]Coceira.[can_scratch ? " You scratch it." : ""]"))
