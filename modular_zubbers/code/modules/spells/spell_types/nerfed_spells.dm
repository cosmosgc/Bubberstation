...
/datum/action/cooldown/spell/pointed/mind_transfer/is_valid_target(atom/cast_on)
	...
	if(living_target.key)
		to_chat(owner, span_warning("[living_target.p_They()] [living_target.p_do()]Não parece ter uma mente vazia para trocar!"))
		return FALSE
...
/datum/spellbook_entry/lichdom
	...
	desc = "Um pacto necrômano infernal que pode unir sua alma para sempre a um item volumosos de sua escolha, transformando você em um Lich imortal. Enquanto o item permanecer intacto, você reviverá da morte, não importa as circunstâncias. Seja cauteloso - com cada reavivamento, seu corpo ficará mais fraco, e será mais fácil para os outros encontrar seu item de poder. Note que a natureza ardente do feitiço também requer que ele seja lançado em um item não à prova de fogo, já que a magia não pode penetrar assim."

/datum/action/cooldown/spell/lichdom
	desc = "Um feitiço ardente das profundezas do inferno que liga sua alma a um item em suas mãos. Amarrar sua alma a um item o transformará em um Lich imortal. Enquanto o item permanecer intacto, você reviverá da morte, não importa as circunstâncias. Não pode ser usado em objetos menores que objetos volumosos ou à prova de fogo."
