		...
		if("track_action")
			switch(params["action"])
				if("set_pnts")
					...
					var/num = tgui_input_number(					usr, 					"Set [track_to_adjust] track points",
					title = "Pontos de trilha", 					default = event_track_points[track_to_adjust], 					max_value = point_thresholds[track_to_adjust]*5, 					)
					if(isnull(num))
						return
